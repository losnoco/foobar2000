...
static SZ_RESULT SzReadTime(CSzData *sd, UInt32 numFiles, CFileItem *files, UInt64 type, ISzAlloc * alloc)
{
	UInt32 i;
	Byte *boolVector;
	RINOK(SzReadBoolVector2(sd, numFiles, &boolVector, alloc->Alloc));
		...
		ISzAlloc *allocTemp)
{
	...
	while(1)
	{
		...
		switch((int)type)
		{
			...
			case k7zIdLastWriteTime:
			{
				RINOK(SzReadTime(sd, numFiles, files, type, allocTemp));
				break;
			}
			default:
			{
