...
Archive::Archive( File_Reader* in, Rar_Error_Handler* eh ) : Raw( in, eh ), NewLhd( eh ), SubHead( eh )
{
	error_handler = eh;
	rar_reader = in;
	OldFormat=false;
...
RawRead::RawRead( File_Reader* in, Rar_Error_Handler* eh ) : Data( eh )
{
	rar_reader = in;
	Reset();
}
...
void RawRead::Read( int count )
{
	if ( count )
	{
		...
		DataSize += rar_reader->read_avail( &Data [DataSize], count );
	}
}
