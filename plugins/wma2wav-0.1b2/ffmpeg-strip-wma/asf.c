...
static unsigned utf8_encode_char(unsigned wide,char * target)
{
	unsigned int count;
...
static unsigned utf16_decode_char(const uint16_t * src,unsigned * out)
{
	unsigned rv = 0;
...
static unsigned convert_utf16_to_utf8(const uint16_t * src,char * dst,unsigned len)
{
	unsigned rv = 0;
	while(*src && len)
	{
...
static void tag_recode(char *before, int len)
{
	...
	rv = convert_utf16_to_utf8(src, dest, len >> 1);

	len = min((unsigned)len - 1, rv);
...
static int64_t asf_read_pts(AVFormatContext *s, int64_t *ppos, int stream_index)
{
    ...
    int64_t pos= *ppos;
    int i;
    int64_t * start_pos = malloc(s->nb_streams * sizeof(int64_t));
