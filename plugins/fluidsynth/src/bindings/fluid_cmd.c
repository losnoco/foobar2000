...
{
  FLUID_TCHAR buf[1024], buf2[1024];
  int id;
  int reset = 1;
  int offset = 0;
...
FLUID_TCHAR*
fluid_expand_path(FLUID_TCHAR* path, FLUID_TCHAR* new_path, int len)
{
#if defined(WIN32) || defined(MACOS9)
  FLUID_TCSNCPY(new_path, path, len - 1);
#else
  if ((path[0] == '~') && (path[1] == '/')) {
    char* home = getenv("HOME");
    if (home == NULL) {
