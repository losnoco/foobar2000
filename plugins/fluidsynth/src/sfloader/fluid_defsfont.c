...
int fluid_defsfont_load(fluid_defsfont_t* sfont, const FLUID_TCHAR* file)
{
  ...
  sfont->filename = FLUID_MALLOC(sizeof(FLUID_TCHAR) + FLUID_TCSLEN(file) * sizeof(FLUID_TCHAR));
  if (sfont->filename == NULL) {
    FLUID_LOG(FLUID_ERR, "Out of memory");
    return FLUID_FAILED;
  }
  FLUID_TCSCPY(sfont->filename, file);

  sfont->file_handle = FLUID_TFOPEN(file, "rb");
  if (sfont->file_handle == NULL) {
    FLUID_LOG(FLUID_ERR, "Couldn't open soundfont file");
    return FLUID_FAILED;
  }
...
SFData *
sfload_file (const FLUID_TCHAR * fname)
{
  SFData *sf = NULL;
  FILE *fd;
  int fsize = 0;
  ...
  if (!err)
    {
      ...
      sf->fname = FLUID_TCSDUP (fname);	/* copy file name */
      sf->sffd = fd;
    }
