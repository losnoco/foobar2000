...
namespace
{
  String readEncodedText(ID3_Reader& reader, size_t len, ID3_TextEnc enc)
  {
    ...
    return io::readUnicodeText(reader, len, enc == ID3TE_UTF16BE);
  }
  ...
  String readEncodedString(ID3_Reader& reader, ID3_TextEnc enc)
  {
    ...
    return io::readUnicodeString(reader, enc == ID3TE_UTF16BE);
  }

  size_t writeEncodedText(ID3_Writer& writer, String data, ID3_TextEnc enc, bool useBOM)
  {
    if (ID3TE_IS_SINGLE_BYTE_ENC(enc))
    {
      return io::writeText(writer, data);
    }
    return io::writeUnicodeText(writer, data, useBOM, enc == ID3TE_UTF16BE);
  }

  size_t writeEncodedString(ID3_Writer& writer, String data, ID3_TextEnc enc, bool useBOM)
  {
    if (ID3TE_IS_SINGLE_BYTE_ENC(enc))
    {
      return io::writeString(writer, data);
    }
    return io::writeUnicodeString(writer, data, useBOM, enc == ID3TE_UTF16BE);
  }
}
...
{
  ID3_TextEnc enc = this->GetEncoding();
  bool useBOM = this->GetBOM();

  if (_flags & ID3FF_CSTR)
  {
    writeEncodedString(writer, _text, enc, useBOM);
  }
  else
  {
    writeEncodedText(writer, _text, enc, useBOM);
  }
  _changed = false;
};
