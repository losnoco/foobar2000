...
String io::readUnicodeString(ID3_Reader& reader, bool assume_bigendian)
{
  String unicode;
...
String io::readUnicodeText(ID3_Reader& reader, size_t len, bool assume_bigendian)
{
  String unicode;
  ID3_Reader::char_type ch1, ch2;
...
size_t io::writeUnicodeString(ID3_Writer& writer, String data, bool bom, bool bigendian)
{
  size_t size = writeUnicodeText(writer, data, bom, bigendian);
  unicode_t null = NULL_UNICODE;
  writer.writeChars((const unsigned char*) &null, 2);
  return size + 2;
}

size_t io::writeUnicodeText(ID3_Writer& writer, String data, bool bom, bool bigendian)
{
  ID3_Writer::pos_type beg = writer.getCur();
  ...
  if (bom)
  {
    ...
    unicode_t BOM = bigendian ? 0xFFFE : 0xFEFF;
    writer.writeChars((const unsigned char*) &BOM, 2);
  }
  const uchar * udata = (uchar *) data.c_str();
  if (bigendian)
  {
    for (size_t i = 0; i < size; i += 2)
    {
      unicode_t ch = (udata[i] << 8) | udata[i+1];
  ...
  else
  {
    for (size_t i = 0; i < size; i += 2)
    {
      unicode_t ch = udata[i] | (udata[i+1] << 8);
      writer.writeChars((const unsigned char*) &ch, 2);
    }
  }
  return writer.getCur() - beg;
