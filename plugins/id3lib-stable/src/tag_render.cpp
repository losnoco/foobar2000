...
void id3::v2::render(ID3_Writer& writer, const ID3_TagImpl& tag)
{
  ...
  ID3D_NOTICE( "id3::v2::render(): rendering, spec " << (int)tag.GetSpec() );
  ID3_TagHeader hdr;
...
size_t ID3_TagImpl::Size() const
{
  ...
  if (this->GetUnsync())
  {
    bytesUsed += frameBytes / 3;
  }
    
  bytesUsed += this->PaddingSize(bytesUsed - hdr.Size()); // spoony fucking bard, Padding already adds the header size
  return bytesUsed;
}
