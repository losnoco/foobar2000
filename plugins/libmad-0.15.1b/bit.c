...
void mad_bit_wrap_init(struct mad_bitwrapptr * bitptr, unsigned char const * byte, unsigned int ptr, unsigned int mask)
{
  bitptr->byte = byte;
  bitptr->ptr = ptr;
  bitptr->cache = 0;
...
unsigned int mad_bit_wrap_length(struct mad_bitwrapptr const *begin,
			    struct mad_bitwrapptr const *end)
{
  int whole = ((end->ptr - begin->ptr) & begin->mask) - 1;
  return begin->left +
    CHAR_BIT * whole + (CHAR_BIT - end->left);
}

void mad_bit_wrap_skip(struct mad_bitwrapptr * bitptr, unsigned int len)
{
  bitptr->ptr = bitptr->ptr + len / CHAR_BIT;
  bitptr->left -= len % CHAR_BIT;
  ...
  if (bitptr->left < CHAR_BIT)
    bitptr->cache = bitptr->byte[bitptr->ptr & bitptr->mask];
}

unsigned long mad_bit_wrap_read(struct mad_bitwrapptr *bitptr, unsigned int len)
{
  register unsigned long value;
  ...
  while (len >= CHAR_BIT) {
    value = (value << CHAR_BIT) | bitptr->byte[(bitptr->ptr++) & bitptr->mask];
    len  -= CHAR_BIT;
  }
