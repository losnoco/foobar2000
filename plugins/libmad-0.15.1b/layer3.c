 ...
 */
static
unsigned int III_scalefactors_lsf(struct mad_bitwrapptr *ptr,
				  struct channel *channel,
				  struct channel *gr1ch, int mode_extension)
{
  struct mad_bitwrapptr start;
  ...
  if (!((mode_extension & I_STEREO) && gr1ch)) {
	...
	channel->scalefac[n++] = mad_bit_wrap_read(ptr, slen[part]);
    }
    ...
    for (part = 0; part < 4; ++part) {
      ...
      for (i = 0; i < nsfb[part]; ++i) {
	is_pos = mad_bit_wrap_read(ptr, slen[part]);
  ...
  return mad_bit_wrap_length(&start, ptr);
}
 ...
 */
static
unsigned int III_scalefactors(struct mad_bitwrapptr *ptr, struct channel *channel,
			      struct channel const *gr0ch, unsigned int scfsi)
{
  ...
  if (channel->block_type == 2) {
    ...
    while (nsfb--)
      channel->scalefac[sfbi++] = mad_bit_wrap_read(ptr, slen1);

    nsfb = 6 * 3;
    while (nsfb--)
      channel->scalefac[sfbi++] = mad_bit_wrap_read(ptr, slen2);
 ...
 */
static
enum mad_error III_huffdecode(struct mad_bitwrapptr *ptr, mad_fixed_t xr[576],
			      struct channel *channel,
			      ...
			      unsigned int part2_length)
{
  ...
  peek = *ptr;
  mad_bit_wrap_skip(ptr, bits_left);

  /* align bit reads to byte boundaries */
  cachesz  = mad_bit_bitsleft(&peek);
  cachesz += ((32 - 1 - 24) + (24 - cachesz)) & ~7;

  bitcache   = mad_bit_wrap_read(&peek, cachesz);
  bits_left -= cachesz;
  ...
  /* big_values */
  {
	...
	bitcache   = (bitcache << bits) | mad_bit_wrap_read(&peek, bits);
	cachesz   += bits;
	bits_left -= bits;
      }
	...
	case 15:
	  if (cachesz < linbits + 2) {
	    bitcache   = (bitcache << 16) | mad_bit_wrap_read(&peek, 16);
	    cachesz   += 16;
	    bits_left -= 16;
	  }
 ...
 */
static
enum mad_error III_decode(struct mad_bitwrapptr *ptr, struct mad_frame *frame,
...
int mad_layer_III(struct mad_stream *stream, struct mad_frame *frame)
{
  ...
  md_len = (data_bitlen + 7) / 8 - si.main_data_begin;/* - next_md_begin;*/
  if (md_len >= 0x80000000) md_len = 0;

  /*frame_used = 0;*/

  if (si.main_data_begin == 0) {
    //ptr = stream->ptr;
    wrapptr.byte = stream->ptr.byte;
	wrapptr.cache = stream->ptr.cache;
	wrapptr.left = stream->ptr.left;
	wrapptr.ptr = 0;
	wrapptr.mask = 0xFFFFFFFF;
    
	stream->md_len = 0;
	stream->md_write_ptr = 0;
    ...
    if (si.main_data_begin > stream->md_len) {
      ...
      mad_bit_wrap_init(&wrapptr,
          *stream->main_data, stream->md_write_ptr - si.main_data_begin, MAD_BUFFER_MDMASK);
		  
      src = mad_bit_nextbyte(&stream->ptr);
      stream->md_len += dsize;
      if (stream->md_len > MAD_BUFFER_MDLEN) stream->md_len = MAD_BUFFER_MDLEN;
      while (dsize > 0)
      {
        int delta = MAD_BUFFER_MDLEN - stream->md_write_ptr;
        if (delta > dsize) delta = dsize;
        memcpy(*stream->main_data + stream->md_write_ptr, src, delta);
        src += delta;
        dsize -= delta;
        stream->md_write_ptr = (stream->md_write_ptr + delta) & MAD_BUFFER_MDMASK;
      }
    }
  }
  ...
  if (result == 0) {
    error = III_decode(&wrapptr, frame, &si, nch);
    if (error) {
      stream->error = error;
      result = -1;
    }
    ...
    stream->anc_ptr.byte    = wrapptr.byte + (wrapptr.ptr & wrapptr.mask);
	stream->anc_ptr.cache   = wrapptr.cache;
	stream->anc_ptr.left    = wrapptr.left;
...
# endif
  ...
  /* one frame at a time, no frame can ever reference past previous available frames,
     above code will toss current frame into this buffer automatically, etc... bah. */
  /* okay... data_bitlen is how many data bits are used by the current block, including
     the reservoir, so we discount that much from the current block of data and chuck
	 that into the reservoir... */
  {
	  int ssize = stream->next_frame - stream->this_frame - frame_space;
	  int dsize = md_len;
	  int framesize = stream->next_frame - stream->this_frame;
	  const unsigned char * src;
	  int bytes;
	  bytes = framesize - (ssize + dsize);
	  src = stream->next_frame - bytes;
	  stream->md_len += bytes;
	  if (stream->md_len > MAD_BUFFER_MDLEN) stream->md_len = MAD_BUFFER_MDLEN;
	  while (bytes > 0)
	  {
