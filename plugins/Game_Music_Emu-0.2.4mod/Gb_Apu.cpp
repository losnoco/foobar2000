...
void Gb_Apu::write_register( gb_time_t time, gb_addr_t addr, int data )
{
	...
	{
		if ( oscs [ 0 ]->global_volume != ( data & 7 ) )
		{
			bool enabled = false;
			int old_volume = oscs [ 0 ]->global_volume;
			int new_volume = data & 7;
			for ( int i = 0; i < osc_count; i++ )
			{
				Gb_Osc& osc = *oscs [i];
				if ( osc.enabled )
				{
					if ( osc.last_amp )
					{
						int new_amp = osc.last_amp * new_volume / osc.global_volume;
						if ( osc.output )
							square_synth.offset( time, new_amp - osc.last_amp, osc.output );
						osc.last_amp = new_amp;
					}
					if ( osc.volume ) enabled = true;
				}
				osc.global_volume = new_volume;
			}

			if ( ! enabled )
			{
				Blip_Buffer* output = oscs[ 0 ]->outputs[ 3 ];
				if ( output )
				{
					square_synth.offset( time, ( new_volume - old_volume ) * 15 * 2, output );
				}
			}
		}
	}
	...
	{
		if (! (regs [0xff26 - start_addr] & 0x80))
		{
			for ( int i = 0; i < osc_count; i++ )
			{
				Gb_Osc& osc = *oscs [i];
				if ( osc.enabled )
				{
					...
					{
						if ( osc.output )
							square_synth.offset( time, -osc.last_amp, osc.output );
						osc.last_amp = 0;
					}
					osc.enabled = false;
				}
