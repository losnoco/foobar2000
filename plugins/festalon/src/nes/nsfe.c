...
int LoadNSFE(FESTALON *fe, FESTALON_NSF *nfe, uint8 *buf, int32 size, int info_only)
{
 ...
 while(size)
 {
  ...
  else if(!memcmp(tb, "plst", 4))
  {
   if ( chunk_size )
   {
    fe->PlaylistSize = chunk_size;
	fe->Playlist = malloc( chunk_size );
	memcpy( fe->Playlist, buf, chunk_size );
   }
  }
  ...
  {
   //printf("Boop: %.4s\n",tb);
   if ( ! nfe->NSFExtra ) nfe->NSFExtra = malloc( 8 + chunk_size );
   else nfe->NSFExtra = realloc(nfe->NSFExtra, nfe->NSFExtraSize + 8 + chunk_size);
...
uint8 *FESTAI_CreateNSFE(FESTALON *fe, FESTALON_NSF *nfe, uint32 *totalsize)
{
 ...
 #define ANED(data,size){ \
  ...
  {
   BEGIN_CHUNK("plst");
   ANED(fe->Playlist,fe->PlaylistSize);
   END_CHUNK();
  }
