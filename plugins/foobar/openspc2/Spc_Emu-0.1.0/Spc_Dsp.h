...
public:
	...
	// Run DSP for 'count' samples. Write resulting samples to 'buf' if it is not NULL.
	void run( long count, short* buf );
	
	// Read/write register 'n'.
...
private:
	...
	// fir_buf [i + 8] == fir_buf [i], to avoid having wrap checking in FIR code
	short fir_buf [16] [2];
	...
	static const short gauss [256] [2];
	...
	inline int Spc_Dsp::read( int i ) {
		assert(( "Spc_Dsp::read(): register out of range", (unsigned) i < register_count ));
		return reg [i];
	}

	inline void Spc_Dsp::write( int i, int data ) {
		assert(( "Spc_Dsp::write(): register out of range", (unsigned) i < register_count ));
		reg [i] = data;
		if ( (i & 0x0f) == 0x0f )
			fir_coeff [i >> 4] = (int8_t) data;
	}
