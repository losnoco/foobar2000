...
void Spc_Dsp::run( long count, short* out_buf )
{
	...
	while ( --count >= 0 )
	{
		...
		if ( g.noise_enables ) {
			// noise doesn't seem to be used by anything
			noise_count -= env_rates [g.flags & 0x1F];
			if ( noise_count <= 0 ) {
				noise_count = env_rate_init;
				noise_amp = (noise & 1) ? noise : -noise; // is amp shifted right at all?
				// noise algorithm verified (same as NES except for noise_amp calc)
		...
		for ( int vidx = 0; vidx < voice_count; vidx++ )
		{
			...
			for ( int n = voice.fraction >> 12; --n >= 0; )
			{
				...
				switch ( (voice.block_header >> 2) & 3 )
				{
					...
					case 2:
						// a [0] = 1.0, b [1] = 0.953125, b [2] = -0.46875
						delta += smp1;
						delta += (-(smp1 + (smp1 >> 1))) >> 5;
						delta -= smp2 >> 1;
						delta += smp2 >> 5;
					...
					case 3:
						// a [0] = 1.0, b [1] = 0.8984375, b [2] = -0.40625
						delta += smp1;
				...
				voice.interp2 = smp2;
				voice.interp1 = smp1;
				voice.interp0 = short (clamp_16( delta ) * 2); // sign-extend
			}
			...
			// output
			int left = (raw_voice.left_vol * output) >> 7;
			int right = (raw_voice.right_vol * output) >> 7;
			if ( g.echo_ons & vbit ) {
				echol += left;
				echor += right;
			}
		...
		// main volume control
		outl = (outl * g.left_volume) >> 7;
		outr = (outr * g.right_volume) >> 7;
		
		// Echo FIR filter
		
		// read feedback from echo buffer
		int echo_ptr = this->echo_ptr;
		uint8_t* buf = &ram [(g.echo_page * 0x100 + echo_ptr) & 0xFFFF];
		echo_ptr += 4;
		if ( echo_ptr >= (g.echo_delay & 15) * 0x800 )
			echo_ptr = 0;
		int fb_left = (short) GET_LE16( buf ); // sign-extend
		int fb_right = (short) GET_LE16( buf + 2 ); // sign-extend
		...
		// overlap calculations with comparisons
		outl += (fb_left * g.left_echo_volume) >> 14;
		
		// echo buffer feedback
		if ( !(g.flags & 0x20) ) {
			echol += (fb_left * g.echo_feedback) >> 14;
			echor += (fb_right * g.echo_feedback) >> 14;
			SET_LE16( buf, clamp_16( echol ) );
			SET_LE16( buf + 2, clamp_16( echor ) );
		}
		
		outr += (fb_right * g.right_echo_volume) >> 14;
