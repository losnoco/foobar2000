...
// Most common instructions used by SPC files:
//
// 15%  0xF0    BEQ rel
//  8%  0xE4    MOV A,dp
//  4%  0xF5    MOV A,abs+X
...
const unsigned char Spc_Cpu::CycleTable[256] = {
...
Spc_Cpu::Spc_Cpu( uint8_t* ram_, Spc_Emu* e ) : ram( ram_ ), emu( *e )
{
	remain_ = 0;
...
unsigned Spc_Cpu::mem_bit( spc_addr_t pc ) {
	...
	unsigned t = READ( addr & 0x1fff );
	return (t << (8 - (addr >> 13))) & 0x100;
}
...
spc_time_t Spc_Cpu::run( spc_time_t cycle_count )
{
	...
	// processors have. Wrap-around isn't performed.
	#define PUSH( v )       (*--sp = (v))
...
branch_taken_loop: // taken branch (displacement already in 'data')
	pc += (signed char) data; // sign-extend
	remain_ -= 2;
inc_pc_loop: // end of instruction with an operand
	pc++;
loop:
	
	// Be sure all registers are valid
	...
	// read opcode and first operand
#if !BLARGG_NONPORTABLE
	unsigned opcode = READ_PROG( pc );
	pc++;
	data = READ_PROG( pc );
	
	// read them with one 16-bit access and extract based on byte order
#elif BLARGG_BIG_ENDIAN
	data = *(unsigned short*) &READ_PROG( pc );
	pc++;
	...
	// ends with data set to the address 
	...
	#define CASE( n ) case n: case (n) + 0x20:
		...
		case 0x88: // ADC imm
			...
			nz = a;
		adc_data: {
			// to do: rewrite this more compactly
			if ( opcode & 0x20 )
				data ^= 0xff; // SBC
			int carry = (c >> 8) & 1;
			int ov = nz ^ 0x80;
			...
			status &= ~(st_v | st_h);
			// temporary works around CodeWarrior optimizer bug
			unsigned temp = (nz & 15) - carry - prev;
			temp = (temp >> 1) & st_h;
			status |= temp;
			
			ov += carry;
			ov += (signed char) data; // sign-extend
			status |= (ov >> 2) & st_v;
	...
	// 17. OTHER COMMANDS.
		...
		/*case 0xFF: // STOP
			...
			{
				pc &= 0xFFFF;
				remain_ -= 3; // rather than fetching from CycleCount
			}
			else
			{
				pc--;
