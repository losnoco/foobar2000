...
/* Unpack SPC given its data and shared data file both read into memory. The output
   buffer must be at least 0x10180 bytes. *Out_size is set to the number of bytes
   actually filled. Return NULL on success, otherwise error string. */
...
/* Read a packed SPC given the ungzipped spc data and a path to the shared data
   ...
   number of bytes actually filled. Return NULL on success, otherwise error string. */
...
struct packed_spc_t {
	char            header [0x100]; /* 0xD8.0 set if packed */
	char            format;         /* 'P' */
