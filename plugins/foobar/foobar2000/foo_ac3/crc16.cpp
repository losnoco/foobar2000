...
void hasher_crc16::process(const void * p_buffer, size_t p_bytes)
{
	uint16_t crc = state;
	const uint8_t * data = static_cast<const uint8_t *>( p_buffer );
	while ( p_bytes >= 8 )
	{
		crc ^= data[0] << 8 | data[1];

		crc = crc16_table[7][crc >> 8] ^ crc16_table[6][crc & 0xFF] ^
		      crc16_table[5][data[2]]  ^ crc16_table[4][data[3]] ^
		      crc16_table[3][data[4]]  ^ crc16_table[2][data[5]] ^
		      crc16_table[1][data[6]]  ^ crc16_table[0][data[7]];

		data += 8;
		p_bytes -= 8;
	}
	while ( p_bytes -- )
		crc = (crc << 8) ^ crc16_table[0][(crc >> 8) ^ *data++];
	state = crc;
}