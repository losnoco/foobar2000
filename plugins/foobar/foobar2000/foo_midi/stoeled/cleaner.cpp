...
//extern cfg_int cfg_hack_xg_drums;
...
//	UINT dm_vol;
	...
	bool f2,tr1,dm,only_ins,ins_no_lsb,cp,emidi;
	bool hasnotes[16];
	...
	BOOL event_ok(BYTE e,BYTE* p)
	{
		...
		if (_c==0xB0)
		{
			//if (cfg_hack_xg_drums && ch==9 && p[1]==0 && (p[0]==0 || p[0]==0x20)) return 0;
		...
		{
			if (only_ins) check_ins(ins[ch].b[2],ins[ch].b[1],ins[ch].b[0],p[0],ch==9,ch);
		}
...
bool CCleaner::do_emidi(UINT n)
{
	...
	while(t->pos<t->sz)
	{
		...
		else
		{
			...
			if ((ce&0xF0) == 0xB0 && ((t->sz - t->pos) >= 2) &&
				(t->data[t->pos] == 110 || t->data[t->pos] == 111))
			{
				if (t->data[t->pos] == 110)
				{
					if (t->data[t->pos+1] != 0 &&
						t->data[t->pos+1] != 127) return true;
				}
				else
				{
					if (t->data[t->pos+1] == 0) return true;
				}
			}
...
void CCleaner::AddEvent(BYTE ev,BYTE* data)
{
	...
	if (!t)
	{
		t=out_trax[nt]=(TRACK*)malloc(64+0x1000);
		if (!t) return;
		ZeroMemory(t,16);
		t->sz=0x1000;
		t->tm=0;
	...
	{
		t->sz*=2;
		out_trax[nt]=(TRACK*)realloc(t,64+t->sz);
		if (!out_trax[nt])
		{
			free(t);
			return;
	...
	if (!cp || ev!=t->le) {t->data[t->pos++]=ev;t->le=ev;}
	t->data[t->pos++]=data[0];
...
int CCleaner::Run(MIDI_file* _mf,DWORD _md,void ** out_data,int * out_size)
{
	...
	cp=(_md&CLEAN_REDUCE)?1:0;
	emidi=(_md&CLEAN_EMIDI)?1:0;
	...
	if (emidi)
	{
		...
		for (n=0,o=0;n<ntrax;n++)
		{
			if (in_trax[n])
			{
				if (n > o) in_trax[o] = in_trax[n];
				o++;
			}
		}
