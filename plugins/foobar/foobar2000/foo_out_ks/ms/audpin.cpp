...
UINT CKsAudPin::GetMaxBitsPerSample()
{
    TRACE_ENTER();
    ...
    while( m_listDataRange.GetNext( listPosRange, &pKSDATARANGE_AUDIO )  )
    {
        ...
        ||  KSDATAFORMAT_TYPE_AUDIO == pKSDATARANGE_AUDIO->DataRange.MajorFormat )
        {
            ...
            GUID guidFormat = {DEFINE_WAVEFORMATEX_GUID(WAVE_FORMAT_PCM)};
            ...
            ||  guidFormat == pKSDATARANGE_AUDIO->DataRange.SubFormat )
            {
                ...
                {
					if (retval < pKSDATARANGE_AUDIO->MaximumBitsPerSample) retval = pKSDATARANGE_AUDIO->MaximumBitsPerSample;
                }
            }
        }
    }
