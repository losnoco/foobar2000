...
public:
	...
	enum {FOOBAR2000_CLIENT_VERSION_COMPATIBLE = 67, FOOBAR2000_CLIENT_VERSION = 67};	//changes everytime global compatibility is broken
	virtual unsigned FB2KAPI get_version() = 0;
	virtual pservice_factory_base FB2KAPI get_service_list() = 0;

	virtual t_io_result FB2KAPI get_config(stream_writer * p_stream,abort_callback & p_abort) = 0;
	virtual t_io_result FB2KAPI set_config(stream_reader * p_stream,abort_callback & p_abort) = 0;
...
public:
	...
	virtual bool FB2KAPI service_enum_create(service_ptr_t<service_base> & p_out,service_class_ref p_class,unsigned p_index) = 0;
	virtual unsigned FB2KAPI service_enum_get_count(service_class_ref p_class) = 0;
