...
public:
	...
	virtual contextmenu_item_node_root * instantiate_item(unsigned p_index,const list_base_const_t<metadb_handle_ptr> & p_data,const GUID & p_caller) = 0;
...
private:
	...
	public:
		...
		bool get_display_data(pfc::string_base & p_out,unsigned & p_displayflags,const list_base_const_t<metadb_handle_ptr> & p_data,const GUID & p_caller) {return m_owner->get_display_data(m_index,p_data,p_out,p_displayflags,p_caller);}
		void execute(const list_base_const_t<metadb_handle_ptr> & p_data,const GUID & p_caller) {m_owner->context_command(m_index,p_data,p_caller);}
...
protected:
	contextmenu_item_node_root * instantiate_item(unsigned p_index,const list_base_const_t<metadb_handle_ptr> & p_data,const GUID & p_caller)
	{
...
public:
	...
	virtual void get_item_name(unsigned p_index,pfc::string_base & p_out)=0;
	virtual void get_item_default_path(unsigned p_index,pfc::string_base & p_out) = 0;
	virtual void context_command(unsigned p_index,const list_base_const_t<metadb_handle_ptr> & p_data,const GUID& p_caller)=0;
	virtual bool context_get_display(unsigned p_index,const list_base_const_t<metadb_handle_ptr> & p_data,pfc::string_base & p_out,unsigned & p_displayflags,const GUID & p_caller) {
