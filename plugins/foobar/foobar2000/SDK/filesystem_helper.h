...
public:
	...
	{
		unsigned base = m_output.get_size();
		if (base + p_bytes < base) return io_result_error_out_of_memory;
		if (!m_output.set_size(base + p_bytes)) return io_result_error_out_of_memory;
...
public:
	...
	inline unsigned get_remaining() const {return m_remaining;}

	t_io_result flush_remaining(abort_callback & p_abort)
	{
