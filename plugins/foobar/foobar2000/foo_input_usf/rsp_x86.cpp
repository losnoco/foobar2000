...
void RSPX86::RSPAdcX86regToVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x11,x86reg,Variable)
}

void RSPX86::RSPAdcConstToVariable(void *Variable, uint8_t Constant) {
	RSPBreakPoint();
...
void RSPX86::RSPAdcX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x13,Destination,Source);
}

void RSPX86::RSPAddConstToVariable (uint32_t Const, void *Variable) {
	OPCODE_REG_VARIABLE(,8,0x81,OP_D0,Variable)
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPAddConstToX86Reg64 (int32_t x86reg, uint32_t Const) {
...
void RSPX86::RSPAddConstToX86Reg (int32_t x86reg, uint32_t Const) {
	if ((Const & 0xFFFFFF80) != 0 && (Const & 0xFFFFFF80) != 0xFFFFFF80) {
...
void RSPX86::RSPAddQwordToX86Reg (int32_t x86reg, uint64_t Const) {
#ifdef USEX64
...
void RSPX86::RSPAddVariableToX86reg(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x3,x86reg,Variable)
}

void RSPX86::RSPAddX86regToVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x1,x86reg,Variable)
}

void RSPX86::RSPAddX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x3,Destination,Source);
}

void RSPX86::RSPAndConstToVariable (uint32_t Const, void *Variable) {
	OPCODE_REG_VARIABLE(,8,0x81,OP_D4,Variable)
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPAndConstToX86Reg(int32_t x86reg, uint32_t Const) {
	if ((Const & 0xFFFFFF80) != 0 && (Const & 0xFFFFFF80) != 0xFFFFFF80) {
...
void RSPX86::RSPAndVariableDispToX86Reg(void *Variable, int32_t x86reg, int32_t AddrReg, int32_t Multiplier) {
#ifdef USEX64
	if(((uintptr_t)Variable - (uintptr_t)TLB_Map) < 0x7FFFFFFF) {
...
void RSPX86::RSPAndVariableToX86Reg(void * Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,8,0x23,x86reg,Variable)
}

void RSPX86::RSPAndX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x21,Source,Destination);
}

void RSPX86::RSPBreakPoint (void) {
	PUTDST8(RSPRecompPos,0xCC);
}

void RSPX86::RSPCall_Direct( void * FunctAddress ) {
	intptr_t disp = 0;
#ifdef USEX64
...
void RSPX86::RSPCompConstToVariable(uint32_t Const, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x81,OP_D7,Variable)
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPCompConstToX86reg(int32_t x86reg, uint32_t Const) {
	if ((Const & 0xFFFFFF80) != 0 && (Const & 0xFFFFFF80) != 0xFFFFFF80) {
...
void RSPX86::RSPCompX86regToVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x3B,x86reg,Variable)
}

void RSPX86::RSPCompX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x3B,Destination,Source);
}

void RSPX86::RSPDecX86reg(int32_t x86reg) {
	OPCODE_REG_REG(8,0xFF,OP_D1,x86reg);
}

void RSPX86::RSPDivX86reg(int32_t x86reg) {
	OPCODE_REG_REG(8,0xF7,OP_D6,x86reg);
}

void RSPX86::RSPidivX86reg(int32_t x86reg) {
	OPCODE_REG_REG(8,0xF7,OP_D7,x86reg);
}

void RSPX86::RSPimulX86reg(int32_t x86reg) {
	OPCODE_REG_REG(8,0xF7,OP_D5,x86reg);
}

void RSPX86::RSPIncX86reg(int32_t x86reg) {
	OPCODE_REG_REG(8,0xFF,OP_D0,x86reg);
}

void RSPX86::RSPJaeLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x73);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJaeLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x830F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJaLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x77);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJaLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x870F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJbLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x72);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJbLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x820F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJecxzLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0xE3);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJeLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x74);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJeLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x840F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJgeLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x8D0F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJgLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x7F);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJgLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x8F0F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJleLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x7E);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJleLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x8E0F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJlLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x7C);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJlLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x8C0F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJzLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x74);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJnzLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x75);
	PUTDST8(RSPRecompPos,Value);
}


void RSPX86::RSPJmpDirectReg( int32_t x86reg ) {
	OPCODE_REG_REG(8,0xff,OP_D4,x86reg);
}

void RSPX86::RSPJmpLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0xEB);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJmpLabel32( uint32_t Value) {
	PUTDST8(RSPRecompPos,0xE9);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJneLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x75);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJneLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x850F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJnsLabel8( uint8_t Value) {
	PUTDST8(RSPRecompPos,0x79);
	PUTDST8(RSPRecompPos,Value);
}

void RSPX86::RSPJnsLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x890F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPJsLabel32(uint32_t Value) {
	PUTDST16(RSPRecompPos,0x880F);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPLeaRegReg(int32_t x86RegDest, int32_t x86RegSrc, int32_t multiplier) {
	OPCODE_REG_BASE_INDEX_SCALE(8,0x8D,x86RegDest,x86_EBP,x86RegSrc,Index[multiplier]);
	PUTDST32(RSPRecompPos,0x00000000);
}

void RSPX86::RSPLeaSourceAndOffset(int32_t x86DestReg, int32_t x86SourceReg, int32_t offset) {
	OPCODE_REG_MREG_IMM32(8,0x8D,x86DestReg,x86SourceReg,offset);
}

void RSPX86::RSPMoveConstByteToVariable (uint8_t Const,void *Variable) {
	OPCODE_REG_VARIABLE(,8,0xC6,OP_D0,Variable)
	PUTDST8(RSPRecompPos,Const);
}

void RSPX86::RSPMoveConstHalfToVariable (uint16_t Const,void *Variable) {
	OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos,0x66),8,0xC7,OP_D0,Variable)
	PUTDST16(RSPRecompPos,Const);
}

void RSPX86::RSPMoveConstHalfToX86regPointer(uint16_t Const, int32_t AddrReg1, int32_t AddrReg2) {
	PUTDST8(RSPRecompPos,0x66);
	OPCODE_REG_BASE_INDEX(8,0xC7,OP_D0,AddrReg1,AddrReg2);
	PUTDST16(RSPRecompPos,Const);
}

void RSPX86::RSPMoveX86regByteToN64Mem(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(,8,0x88,x86reg,AddrReg);
}

void RSPX86::RSPMoveX86regHalfToN64Mem(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(PUTDST8(RSPRecompPos,0x66),8,0x89,x86reg,AddrReg);
}

void RSPX86::RSPMoveX86regToN64Mem(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(,8,0x89,x86reg,AddrReg);
}


void RSPX86::RSPMoveConstToVariable (uint32_t Const,void *Variable) {
	OPCODE_REG_VARIABLE(,8,0xC7,OP_D0,Variable)
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPMoveConstToX86Pointer(uint32_t Const, int32_t X86Pointer) {
	OPCODE_REG_MREG(8,0xC7,OP_D0,X86Pointer);
	PUTDST32(RSPRecompPos,Const);
}


void RSPX86::RSPMoveOffsetToX86reg(uint32_t Const, int32_t x86reg) {
	OPCODE_REG_REG(8,0xC7,OP_D0,x86reg);
	PUTDST32(RSPRecompPos,Const);
}


void RSPX86::RSPMoveConstToX86reg(uint32_t Const, int32_t x86reg) {
	OPCODE_REG_REG(8,0xC7,OP_D0,x86reg);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPMoveConstQwordToX86reg(uintptr_t Const, int32_t x86reg) {
#ifndef USEX64
	OPCODE_REG_REG(8,0xC7,OP_D0,x86reg);
	PUTDST32(RSPRecompPos,Const);
#else
...
void RSPX86::RSPMoveConstByteToX86regPointer(uint8_t Const, int32_t AddrReg1, int32_t AddrReg2) {
	OPCODE_REG_BASE_INDEX(8,0xC6,OP_D0,AddrReg1,AddrReg2)
	PUTDST8(RSPRecompPos,Const);
}

void RSPX86::RSPMoveConstToX86regPointer(uint32_t Const, int32_t AddrReg1, int32_t AddrReg2) {
	OPCODE_REG_BASE_INDEX(8,0xC7,OP_D0,AddrReg1,AddrReg2);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPMoveN64MemToX86reg(int32_t x86reg, int32_t AddrReg) {
#ifdef USEX64
...
void RSPX86::RSPMoveSxByteX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2, int32_t x86reg) {
	OPCODE_REG_BASE_INDEX(16,0xBE0F,x86reg,AddrReg1,AddrReg2);
}

void RSPX86::RSPMoveSxHalfX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2, int32_t x86reg) {
	OPCODE_REG_BASE_INDEX(16,0xBF0F,x86reg,AddrReg1,AddrReg2);
}


void RSPX86::RSPMoveN64MemToX86regByte(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(,8,0x8A,x86reg,AddrReg);
}


void RSPX86::RSPMoveSxN64MemToX86regByte(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(,16,0xBE0F,x86reg,AddrReg);
}

void RSPX86::RSPMoveSxVariableToX86regHalf(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,16,0xBF0F,x86reg,Variable);
}


void RSPX86::RSPMoveSxN64MemToX86regHalf(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(,16,0xBF0F,x86reg,AddrReg);
}

void RSPX86::RSPMoveZxN64MemToX86regHalf(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_ADDR_DMEM(,16,0xB70F,x86reg,AddrReg);
}

void RSPX86::RSPMoveVariableToX86reg(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,8,0x8B,x86reg,Variable);
}

void RSPX86::RSPMoveX86RegDispToX86Reg(int32_t x86reg, int32_t AddrReg, int32_t IndexReg, int32_t Multiplier) {
	OPCODE_REG_BASE_INDEX_SCALE(8,0x8B,x86reg,AddrReg,IndexReg,Index[Multiplier]);
}

void RSPX86::RSPMoveN64MemDispToX86reg(int32_t x86reg, int32_t AddrReg, uint8_t Disp) {
#ifdef USEX64
	if(((uintptr_t)DMEM - (uintptr_t)TLB_Map) < 0x7FFFFFFF) {
...
void RSPX86::RSPMoveVariableDispToX86Reg(void *Variable, int32_t x86reg, int32_t AddrReg, int32_t Multiplier) {
#ifdef USEX64
	if(((uintptr_t)Variable - (uintptr_t)TLB_Map) < 0x7FFFFFFF) {
...
void RSPX86::RSPMoveN64MemToX86regHalf(int32_t x86reg, int32_t AddrReg)
{
	RSPMoveVariableDispToX86Reg(DMEM,x86reg,AddrReg,1);
}


void RSPX86::RSPMoveVariableToX86regByte(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,8,0x8A,x86reg,Variable);
}

void RSPX86::RSPMoveVariableToX86regHalf(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos,0x66),8,0x8B,x86reg,Variable);
}

void RSPX86::RSPMoveX86regByteToVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x88,x86reg,Variable);
}

void RSPX86::RSPMoveX86regByteToX86regPointer(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_MREG(8,0x88,x86reg,AddrReg);
}

void RSPX86::RSPMoveX86regHalfToVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos,0x66),8,0x89,x86reg,Variable);
}

void RSPX86::RSPMoveX86regHalfToX86regPointer(int32_t x86reg, int32_t AddrReg) {
	PUTDST8(RSPRecompPos,0x66);
	OPCODE_REG_MREG(8,0x89,x86reg,AddrReg);
}

void RSPX86::RSPMoveX86PointerToX86reg(int32_t x86reg, int32_t X86Pointer) {
	OPCODE_REG_MREG(8,0x8B,x86reg,X86Pointer);
}

void RSPX86::RSPMoveX86regPointerToX86reg(int32_t Destination, int32_t AddrReg) {
	OPCODE_REG_MREG(8,0x8B,Destination,AddrReg);
}

void RSPX86::RSPMoveX86regPointerToX86regDisp8(int32_t AddrReg1, int32_t AddrReg2, int32_t x86reg, uint8_t offset) {
	OPCODE_REG_BASE_INDEX_IMM8(8,0x8B,x86reg,AddrReg1,AddrReg2,offset);
}

void RSPX86::RSPMoveX86regToMemory(int32_t x86reg, int32_t AddrReg, uint32_t Disp) {
	OPCODE_REG_MREG_IMM32(8,0x89,x86reg,AddrReg,Disp);
}

void RSPX86::RSPMoveX86regToVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x89,x86reg,Variable);
}

void RSPX86::RSPMoveX86RegToX86Reg(int32_t Source, int32_t Destination) {
	OPCODE_REG_REG(8,0x89,Source|X64_Reg, Destination|X64_Reg);
}

void RSPX86::RSPMoveX86regToX86Pointer(int32_t x86reg, int32_t X86Pointer) {
	OPCODE_REG_MREG(8,0x89,x86reg, X86Pointer);
}

void RSPX86::RSPMoveX86regToX86regPointer(int32_t x86reg, int32_t AddrReg) {
	OPCODE_REG_MREG(8,0x89,x86reg,AddrReg);
}

void RSPX86::RSPMoveZxByteX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2, int32_t x86reg) {
	OPCODE_REG_BASE_INDEX(16,0xB60F,x86reg,AddrReg1,AddrReg2);
}

void RSPX86::RSPMoveZxHalfX86regPointerToX86reg(int32_t AddrReg1, int32_t AddrReg2, int32_t x86reg) {
	OPCODE_REG_BASE_INDEX(16,0xB70F,x86reg,AddrReg1,AddrReg2);
}

void RSPX86::RSPMoveZxVariableToX86regByte(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,16,0xB60F,x86reg,Variable);
}

void RSPX86::RSPMoveZxVariableToX86regHalf(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,16,0xB70F,x86reg,Variable);
}

void RSPX86::RSPMulX86reg(int32_t x86reg) {
	OPCODE_REG_REG(8,0xF7,OP_D4,x86reg);
}

void RSPX86::RSPNotX86Reg(int32_t  x86reg) {
	OPCODE_REG_REG(8,0xF7,OP_D2,x86reg);
}

void RSPX86::RSPOrConstToVariable(uint32_t Const, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x81,OP_D1,Variable);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPOrConstToX86Reg(uint32_t Const, int32_t  x86reg) {
	if ((Const & 0xFFFFFF80) != 0 && (Const & 0xFFFFFF80) != 0xFFFFFF80) {
...
void RSPX86::RSPOrVariableToX86Reg(void * Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,8,0xb,x86reg,Variable);
}

void RSPX86::RSPOrX86RegToVariable(void * Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,8,0x9,x86reg,Variable);
}

void RSPX86::RSPOrX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x0B,Destination,Source);
}

void RSPX86::RSPPushfd() {
	PUTDST8(RSPRecompPos,0x9c);
}

void RSPX86::RSPPopfd() {
	PUTDST8(RSPRecompPos,0x9d);
}

void RSPX86::RSPPopad(void) {
#ifdef USEX64
...
void RSPX86::RSPPush(int32_t x86reg) {
...
void RSPX86::RSPPop(int32_t x86reg) {
#ifdef USEX64
	PUTDST8(RSPRecompPos, 0x40 | ((x86reg & 0x20) >> 5));
#endif
	switch(x86reg&0xf) {
...
void RSPX86::RSPPushImm32(uint32_t Value) {
	PUTDST8(RSPRecompPos,0x68);
	PUTDST32(RSPRecompPos,Value);
}

void RSPX86::RSPRet(void) {
	PUTDST8(RSPRecompPos,0xC3);
}

void RSPX86::RSPSeta(int32_t x86reg) {
	OPCODE_REG_REG(16,0x970F,OP_D0,x86reg);
}

void RSPX86::RSPSetaVariable(void * Variable) {
	OPCODE_REG_VARIABLE(,16,0x970F,OP_D0,Variable);
}

void RSPX86::RSPSetae(int32_t x86reg) {
	OPCODE_REG_REG(16,0x930F,OP_D0,x86reg);
}

void RSPX86::RSPSetb(int32_t x86reg) {
	OPCODE_REG_REG(16,0x920F,OP_D0,x86reg);
}

void RSPX86::RSPSetbVariable(void * Variable) {
	OPCODE_REG_VARIABLE(,16,0x920F,OP_D0,Variable);
}

void RSPX86::RSPSetg(int32_t x86reg) {
	OPCODE_REG_REG(16,0x9F0F,OP_D0,x86reg);
}

void RSPX86::RSPSetgVariable(void * Variable) {
	OPCODE_REG_VARIABLE(,16,0x9F0F,OP_D0,Variable);
}

void RSPX86::RSPSetl(int32_t x86reg) {
	OPCODE_REG_REG(16,0x9C0F,OP_D0,x86reg);
}

void RSPX86::RSPSetlVariable(void * Variable) {
	OPCODE_REG_VARIABLE(,16,0x9C0F,OP_D0,Variable);
}

void RSPX86::RSPShiftLeftSignVariableImmed(void *Variable, uint8_t Immediate) {
	OPCODE_REG_VARIABLE(,8,0xC1,OP_D4,Variable);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPShiftRightSignVariableImmed(void *Variable, uint8_t Immediate) {
	OPCODE_REG_VARIABLE(,8,0xC1,OP_D7,Variable);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPShiftRightUnsignVariableImmed(void *Variable, uint8_t Immediate) {
	OPCODE_REG_VARIABLE(,8,0xC1,OP_D5,Variable);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPSetzVariable(void *Variable) {
	OPCODE_REG_VARIABLE(,16,0x940F,OP_D0,Variable);
}

void RSPX86::RSPSetnzVariable(void *Variable) {
	OPCODE_REG_VARIABLE(,16,0x950F,OP_D0,Variable);
}

void RSPX86::RSPSetleVariable(void *Variable) {
	RSPBreakPoint();
	OPCODE_REG_VARIABLE(,16,0x9E0F,OP_D0,Variable);
}

void RSPX86::RSPSetgeVariable(void *Variable) {
	RSPBreakPoint();
	OPCODE_REG_VARIABLE(,16,0x9D0F,OP_D0,Variable);
}

void RSPX86::RSPSetz(int32_t x86reg) {
	OPCODE_REG_REG(16,0x940F,OP_D0,x86reg);
}

void RSPX86::RSPSetnz(int32_t x86reg) {
	OPCODE_REG_REG(16,0x950F,OP_D0,x86reg);
}

void RSPX86::RSPShiftLeftDouble(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(16,0xA50F,Source,Destination);
}

void RSPX86::RSPShiftLeftDoubleImmed(int32_t Destination, int32_t Source, uint8_t Immediate) {
	OPCODE_REG_REG(16,0xA40F,Source,Destination);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPShiftLeftSign(int32_t x86reg) {
	OPCODE_REG_REG(8,0xD3,OP_D4,x86reg);
}

void RSPX86::RSPShiftLeftSignImmed(int32_t x86reg, uint8_t Immediate) {
	OPCODE_REG_REG(8,0xC1,OP_D4,x86reg);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPShiftRightSign(int32_t x86reg) {
	OPCODE_REG_REG(8,0xD3,OP_D7,x86reg);
}

void RSPX86::RSPShiftRightSignImmed(int32_t x86reg, uint8_t Immediate) {
	OPCODE_REG_REG(8,0xC1,OP_D7,x86reg);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPShiftRightUnsign(int32_t x86reg) {
	OPCODE_REG_REG(8,0xD3,OP_D5,x86reg);
}

void RSPX86::RSPShiftRightDouble(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(16,0xAD0F,Source,Destination);
}

void RSPX86::RSPShiftRightDoubleImmed(int32_t Destination, int32_t Source, uint8_t Immediate) {
	OPCODE_REG_REG(16,0xAC0F,Source,Destination);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPShiftRightUnsignImmed(int32_t x86reg, uint8_t Immediate) {
	OPCODE_REG_REG(8,0xC1,OP_D5,x86reg);
	PUTDST8(RSPRecompPos,Immediate);
}

void RSPX86::RSPSbbConstFromX86Reg (int32_t x86reg, uint32_t Const) {
RSPBreakPoint();
...
void RSPX86::RSPSbbVariableFromX86reg(int32_t x86reg, void * Variable) {
	RSPBreakPoint();
	OPCODE_REG_VARIABLE(,8,0x1B,x86reg,Variable);
}

void RSPX86::RSPSbbX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x1B,Destination,Source);
}

void RSPX86::RSPSubConstFromVariable (uint32_t Const, void *Variable) {
	OPCODE_REG_VARIABLE(,8,0x81,OP_D5,Variable);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPSubConstFromX86Reg (int32_t x86reg, uint32_t Const) {
	if ((Const & 0xFFFFFF80) != 0 && (Const & 0xFFFFFF80) != 0xFFFFFF80) {
...
void RSPX86::RSPSubVariableFromX86reg(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x2B,x86reg,Variable);
}

void RSPX86::RSPSubX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x2B,Destination,Source);
}

void RSPX86::RSPTestConstToX86Reg(uint32_t Const, int32_t x86reg) {
	OPCODE_REG_REG(8,0xF7,OP_D0,x86reg);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPTestVariable(uint32_t Const, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0xf7,OP_D0,Variable);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPTestX86RegToX86Reg(int32_t Destination, int32_t Source) {
	OPCODE_REG_REG(8,0x85,Destination,Source);
}

void RSPX86::RSPTestVariableToX86Reg(uint32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x85,x86reg,Variable);
}

void RSPX86::RSPXorConstToX86Reg(int32_t x86reg, uint32_t Const) {
	if ((Const & 0xFFFFFF80) != 0 && (Const & 0xFFFFFF80) != 0xFFFFFF80) {
...
void RSPX86::RSPXorX86RegToX86Reg(int32_t Source, int32_t Destination) {
	OPCODE_REG_REG(8,0x31,Destination,Source);
}

void RSPX86::RSPXorVariableToX86reg(void *Variable, int32_t x86reg) {
	RSPBreakPoint();
	OPCODE_REG_VARIABLE(,8,0x33,x86reg,Variable);
}


void RSPX86::RSPXorConstToVariable(void *Variable, uint32_t Const) {
...
void RSPX86::RSPAndX86RegToVariable(void * Variable, int32_t x86Reg) {
	OPCODE_REG_VARIABLE(,8,0x21,x86Reg,Variable);
}

void RSPX86::RSPXorX86RegToVariable(void *Variable, int32_t x86reg) {
	OPCODE_REG_VARIABLE(,8,0x31,x86reg,Variable);
}

void RSPX86::RSPTestConstToVariable(uint32_t Const, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0xf7,OP_D0,Variable);
	PUTDST32(RSPRecompPos,Const);
}

void RSPX86::RSPCondMoveGreaterEqual(int32_t Destination, int32_t Source) {
...
void RSPX86::RSPMoveX86regPointerToX86regByte(int32_t Destination, int32_t AddrReg) {
...
void RSPX86::RSPMoveX86regToN64MemDisp(int32_t x86reg, int32_t AddrReg, uint8_t Disp) {
#ifdef USEX64
	if(((uintptr_t)DMEM - (uintptr_t)TLB_Map) < 0x7FFFFFFF) {
...
void RSPX86::RSPOrVariableToX86regHalf(void * Variable, int32_t x86Reg) {
	OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos,0x66),8,0xb,x86Reg,Variable);
}

void RSPX86::RSPAndVariableToX86regHalf(void * Variable, int32_t x86Reg) {
	OPCODE_REG_VARIABLE(PUTDST8(RSPRecompPos,0x66),8,0x23,x86Reg,Variable);
}

void RSPX86::RSPAndX86RegHalfToX86RegHalf(int32_t Destination, int32_t Source) {
	PUTDST8(RSPRecompPos, 0x66);
	OPCODE_REG_REG(8,0x21,Source,Destination);
}

void RSPX86::RSPCondMoveNotEqual(int32_t Destination, int32_t Source) {
	if (ConditionalMove == 0) {
		uint8_t * Jump;
...
void RSPX86::RSPNegateX86reg(int32_t x86reg) {
	// CPU_Message("	  neg %s", x86_Name(int32_t x86reg));
	switch (x86reg&0xf) {
...
void RSPX86::RSPImulX86RegToX86Reg(int32_t Destination, int32_t Source) {
	uint8_t x86Command;
...
void RSPX86::RSPMoveX86RegToX86regPointerDisp ( int32_t Source, int32_t AddrReg, uint8_t Disp ) {
	OPCODE_REG_MREG_IMM8(8,0x89,Source,AddrReg,Disp);
}

void RSPX86::RSPCondMoveGreater(int32_t Destination, int32_t Source) {
	if (ConditionalMove == 0) {
		uint8_t * Jump;
...
void RSPX86::RSPCondMoveLess(int32_t Destination, int32_t Source) {
	if (ConditionalMove == 0) {
		uint8_t * Jump;
...
void RSPX86::RSPMoveSxX86RegPtrDispToX86RegHalf(int32_t AddrReg, uint8_t Disp, int32_t Destination) {
	OPCODE_REG_MREG_IMM8(16,0xBF0F,Destination,AddrReg,Disp);

}


void RSPX86::RSPMoveZxX86RegPtrDispToX86RegHalf(int32_t AddrReg, uint8_t Disp, int32_t Destination) {
	OPCODE_REG_MREG_IMM8(16,0xB70F,Destination,AddrReg,Disp);
}


void RSPX86::RSPMoveX86regHalfToX86regPointerDisp(int32_t Source, int32_t AddrReg, uint8_t Disp) {
	uint8_t x86Amb;
...
void RSPX86::RSPCondMoveEqual(int32_t Destination, int32_t Source) {
	if (ConditionalMove == 0) {
		uint8_t * Jump;
...
void RSPX86::RSPCwd(void) {
	// CPU_Message("	  cwd");
	PUTDST16(RSPRecompPos, 0x9966);
}

void RSPX86::RSPCwde(void) {
	// CPU_Message("	  cwde");
	PUTDST8(RSPRecompPos, 0x98);
}

void RSPX86::RSPSubX86regFromVariable(int32_t x86reg, void * Variable) {
	OPCODE_REG_VARIABLE(,8,0x29,x86reg,Variable);
}

void RSPX86::RSPJumpX86Reg( int32_t x86reg ) {
	// CPU_Message("	  jmp %s",x86_Name(int32_t x86reg));
/*
	switch (int32_t x86reg) {
...
void RSPX86::RSPJgeLabel8(uint8_t Value) {
