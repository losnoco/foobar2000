...
int32_t Rsp::IsOpcodeNop(uint32_t PC) {
	RSPOPCODE RspOp;
...
int32_t Rsp::IsNextInstructionMmx(uint32_t PC) {
	RSPOPCODE RspOp;
...
uint32_t Rsp::WriteToAccum2 (int32_t Location, int32_t PC, int32_t RecursiveCall) {
	RSPOPCODE RspOp;
...
int32_t Rsp::WriteToAccum (int32_t Location, int32_t PC) {
...
int32_t Rsp::WriteToVectorDest2 (uint32_t DestReg, int32_t PC, int32_t RecursiveCall) {
	RSPOPCODE RspOp;
	uint32_t BranchTarget = 0;
	int32_t BranchImmed = 0;
	uint32_t JumpTarget = 0;
...
int32_t Rsp::WriteToVectorDest (uint32_t DestReg, int32_t PC) {
...
int32_t Rsp::UseRspFlags (int32_t PC) {
	RSPOPCODE RspOp;
	int32_t Instruction_State = RSPNextInstruction;
...
int32_t Rsp::IsRegisterConstant (uint32_t Reg, uint32_t * Constant) {
	uint32_t PC = 0;
...
int32_t Rsp::IsOpcodeBranch(uint32_t PC, RSPOPCODE RspOp) {
	switch (RspOp.op) {
	case RSP_REGIMM:
		switch (RspOp.rt) {
		case RSP_REGIMM_BLTZ:
...
void Rsp::GetInstructionInfo(uint32_t PC, RSPOPCODE * RspOp, OPCODE_INFO * info) {
	switch (RspOp->op) {
	case RSP_REGIMM:
		switch (RspOp->rt) {
		case RSP_REGIMM_BLTZ:
...
int32_t Rsp::DelaySlotAffectBranch(uint32_t PC) {
...
int32_t Rsp::CompareInstructions(uint32_t PC, RSPOPCODE * Top, RSPOPCODE * Bottom) {
