...
void Rsp::RSP_Opcode_SLTIU (void) {
	if (RSPOpC.rt == 0) { return; }
...
void Rsp::RSP_Opcode_ANDI ( void ) {
	if (RSPOpC.rt != 0) {
		RSP_GPR[RSPOpC.rt].W = RSP_GPR[RSPOpC.rs].W & RSPOpC.immediate;
	}
}

void Rsp::RSP_Opcode_ORI ( void ) {
	if (RSPOpC.rt != 0) {
		RSP_GPR[RSPOpC.rt].W = RSP_GPR[RSPOpC.rs].W | RSPOpC.immediate;
	}
}

void Rsp::RSP_Opcode_XORI ( void ) {
	if (RSPOpC.rt != 0) {
		RSP_GPR[RSPOpC.rt].W = RSP_GPR[RSPOpC.rs].W ^ RSPOpC.immediate;
	}
}

void Rsp::RSP_Opcode_LUI (void) {
	if (RSPOpC.rt != 0) {
		RSP_GPR[RSPOpC.rt].W = (int16_t)RSPOpC.offset << 16;
	}
}

void Rsp::RSP_Opcode_COP0 (void) {
	(this->*RSP_Cop0[ RSPOpC.rs ])();
}

void Rsp::RSP_Opcode_COP2 (void) {
	(this->*RSP_Cop2[ RSPOpC.rs ])();
}

void Rsp::RSP_Opcode_LB ( void ) {
...
void Rsp::RSP_Special_BREAK ( void ) {
	RSP_Running = 0;
	cpu.SP_STATUS_REG |= (SP_STATUS_HALT | SP_STATUS_BROKE );
	if ((cpu.SP_STATUS_REG & SP_STATUS_INTR_BREAK) != 0 ) {
		cpu.MI_INTR_REG |= R4300i_SP_Intr;
		cpu.CheckInterrupts();
	}
}

void Rsp::RSP_Special_ADD (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].W = RSP_GPR[RSPOpC.rs].W + RSP_GPR[RSPOpC.rt].W;
	}
}

void Rsp::RSP_Special_ADDU (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].UW = RSP_GPR[RSPOpC.rs].UW + RSP_GPR[RSPOpC.rt].UW;
	}
}

void Rsp::RSP_Special_SUB (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].W = RSP_GPR[RSPOpC.rs].W - RSP_GPR[RSPOpC.rt].W;
	}
}

void Rsp::RSP_Special_SUBU (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].UW = RSP_GPR[RSPOpC.rs].UW - RSP_GPR[RSPOpC.rt].UW;
	}
}

void Rsp::RSP_Special_AND (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].UW = RSP_GPR[RSPOpC.rs].UW & RSP_GPR[RSPOpC.rt].UW;
	}
}

void Rsp::RSP_Special_OR (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].UW = RSP_GPR[RSPOpC.rs].UW | RSP_GPR[RSPOpC.rt].UW;
	}
}

void Rsp::RSP_Special_XOR (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].UW = RSP_GPR[RSPOpC.rs].UW ^ RSP_GPR[RSPOpC.rt].UW;
	}
}

void Rsp::RSP_Special_NOR (void) {
	if (RSPOpC.rd != 0) {
		RSP_GPR[RSPOpC.rd].UW = ~(RSP_GPR[RSPOpC.rs].UW | RSP_GPR[RSPOpC.rt].UW);
	}
}

void Rsp::RSP_Special_SLT (void) {
...
void Rsp::RSP_Special_SLTU (void) {
	if (RSPOpC.rd == 0) { return; }
...
void Rsp::RSP_Opcode_BLTZAL ( void ) {
	RSP_NextInstruction = DELAY_SLOT;
	RSP_GPR[31].UW = ( *PrgCount + 8 ) & 0xFFC;
	if (RSP_GPR[RSPOpC.rs].W < 0) {
		RSP_JumpTo = ( *PrgCount + ((int16_t)RSPOpC.offset << 2) + 4 ) & 0xFFC;
...
void Rsp::RSP_Opcode_BGEZAL ( void ) {
	RSP_NextInstruction = DELAY_SLOT;
	RSP_GPR[31].UW = ( *PrgCount + 8 ) & 0xFFC;
	if (RSP_GPR[RSPOpC.rs].W >= 0) {
		RSP_JumpTo = ( *PrgCount + ((int16_t)RSPOpC.offset << 2) + 4 ) & 0xFFC;
...
/************************** Cop0 functions *************************/
void Rsp::RSP_Cop0_MF (void) {
	switch (RSPOpC.rd) {
	case 4: RSP_GPR[RSPOpC.rt].UW = cpu.SP_STATUS_REG; break;
	case 5: RSP_GPR[RSPOpC.rt].UW = cpu.SP_DMA_FULL_REG; break;
	case 6: RSP_GPR[RSPOpC.rt].UW = cpu.SP_DMA_BUSY_REG; break;
	case 7:
		RSP_GPR[RSPOpC.rt].W = 0;
		//RSP_GPR[RSPOpC.rt].W = SP_SEMAPHORE_REG;
		//SP_SEMAPHORE_REG = 1;
		break;
	case 8: RSP_GPR[RSPOpC.rt].UW = cpu.DPC_START_REG ; break;
	case 9: RSP_GPR[RSPOpC.rt].UW = cpu.DPC_END_REG ; break;
	case 10: RSP_GPR[RSPOpC.rt].UW = cpu.DPC_CURRENT_REG; break;
	case 11: RSP_GPR[RSPOpC.rt].W = cpu.DPC_STATUS_REG; break;
	case 12: RSP_GPR[RSPOpC.rt].W = cpu.DPC_CLOCK_REG; break;
	default:
		;//printf("have not implemented RSP MF cpu->CP0 reg (%d)",RSPOpC.rd);
	}
}

void Rsp::RSP_Cop0_MT (void) {
	switch (RSPOpC.rd) {
	case 0: cpu.SP_MEM_ADDR_REG  = RSP_GPR[RSPOpC.rt].UW; break;
	case 1: cpu.SP_DRAM_ADDR_REG = RSP_GPR[RSPOpC.rt].UW; break;
	case 2:
		cpu.SP_RD_LEN_REG = RSP_GPR[RSPOpC.rt].UW;
		RSP_SP_DMA_READ();
		break;
	case 3:
		cpu.SP_WR_LEN_REG = RSP_GPR[RSPOpC.rt].UW;
		RSP_SP_DMA_WRITE();
		break;
	case 4:
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_HALT ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_HALT; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_HALT ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_HALT;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_BROKE ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_BROKE; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_INTR ) != 0) { cpu.MI_INTR_REG &= ~R4300i_SP_Intr; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_INTR ) != 0) { /*printf("SP_SET_INTR");*/  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SSTEP ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SSTEP; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SSTEP ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SSTEP;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_INTR_BREAK ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_INTR_BREAK; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_INTR_BREAK ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_INTR_BREAK;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG0 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG0; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG0 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG0;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG1 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG1; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG1 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG1;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG2 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG2; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG2 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG2;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG3 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG3; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG3 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG3;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG4 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG4; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG4 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG4;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG5 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG5; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG5 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG5;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG6 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG6; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG6 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG6;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_CLR_SIG7 ) != 0) { cpu.SP_STATUS_REG &= ~SP_STATUS_SIG7; }
		if ( ( RSP_GPR[RSPOpC.rt].W & SP_SET_SIG7 ) != 0) { cpu.SP_STATUS_REG |= SP_STATUS_SIG7;  }
		break;
	case 7: cpu.SP_SEMAPHORE_REG = 0; break;
	case 8:
		cpu.DPC_START_REG = RSP_GPR[RSPOpC.rt].UW;
		cpu.DPC_CURRENT_REG = RSP_GPR[RSPOpC.rt].UW;
		break;
	case 9:
		cpu.DPC_END_REG = RSP_GPR[RSPOpC.rt].UW;
		break;
	case 10: cpu.DPC_CURRENT_REG = RSP_GPR[RSPOpC.rt].UW; break;
	case 11:
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_XBUS_DMEM_DMA ) != 0) { cpu.DPC_STATUS_REG &= ~DPC_STATUS_XBUS_DMEM_DMA; }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_SET_XBUS_DMEM_DMA ) != 0) { cpu.DPC_STATUS_REG |= DPC_STATUS_XBUS_DMEM_DMA;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_FREEZE ) != 0) { cpu.DPC_STATUS_REG &= ~DPC_STATUS_FREEZE; }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_SET_FREEZE ) != 0) { cpu.DPC_STATUS_REG |= DPC_STATUS_FREEZE;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_FLUSH ) != 0) { cpu.DPC_STATUS_REG &= ~DPC_STATUS_FLUSH; }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_SET_FLUSH ) != 0) { cpu.DPC_STATUS_REG |= DPC_STATUS_FLUSH;  }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_TMEM_CTR ) != 0) { /* printf("RSP: cpu.DPC_STATUS_REG: DPC_CLR_TMEM_CTR"); */ }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_PIPE_CTR ) != 0) { /*printf("RSP: cpu.DPC_STATUS_REG: DPC_CLR_PIPE_CTR");*/ }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_CMD_CTR ) != 0) { /*printf("RSP: cpu.DPC_STATUS_REG: DPC_CLR_CMD_CTR");*/ }
		if ( ( RSP_GPR[RSPOpC.rt].W & DPC_CLR_CLOCK_CTR ) != 0) { /* printf("RSP: cpu.DPC_STATUS_REG: DPC_CLR_CLOCK_CTR"); */ }
		break;
	default:
...
/************************** Vect functions **************************/
 ...
 Format:  VMULF $v<dest>, $v<s1>, $v<s2>[el]
					 sa	  rd	   rt  rs
					 */
void Rsp::RSP_Vector_VMULF (void) {
...
void Rsp::RSP_Vector_VMULU (void) {
	int32_t count, el, del;
...
void Rsp::RSP_Vector_VMUDL (void) {
	int32_t count, el, del;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VMUDM (void) {
	int32_t count, el, del;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VMUDN (void) {
	int32_t count, el, del;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VMUDH (void) {
	int32_t count, el, del;
...
void Rsp::RSP_Vector_VMACF (void) {
	int32_t count, el, del;
	//UWORD temp, temp2;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VMACU (void) {
	int32_t count, el, del;
...
void Rsp::RSP_Vector_VMACQ (void) {
	int32_t count, el, del;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VMADL (void) {
	int32_t count, el, del;
	REGISTER32 temp, temp2;
...
void Rsp::RSP_Vector_VMADM (void) {
	int32_t count, el, del;
	REGISTER32 temp, temp2;
...
void Rsp::RSP_Vector_VMADN (void) {
	int32_t count, el, del;
	REGISTER32 temp, temp2;
...
void Rsp::RSP_Vector_VMADH (void) {
	int32_t count, el, del;
...
void Rsp::RSP_Vector_VADDC (void) {
	int32_t count, el, del;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VSUBC (void) {
	int32_t count, el, del;
	REGISTER32 temp;
...
void Rsp::RSP_Vector_VNAND (void) {
	int32_t count, el, del;
...
void Rsp::RSP_Vector_VNXOR (void) {
	int32_t count, el, del;
...
void Rsp::RSP_Vector_VRCPL (void) {
	int32_t count, neg;
...
void Rsp::RSP_Vector_VRCPH (void) {
	int32_t count;
...
void Rsp::RSP_Vector_VRSQL (void) {
	int32_t count, neg;
...
void Rsp::RSP_Vector_VRSQH (void) {
	int32_t count;
...
void Rsp::rsp_UnknownOpcode (void) {
