...
void Cpu::PifRamWrite (void) {
	...
	for (CurPos = 0; CurPos < 0x40; CurPos++){
		...
		default:
			if ((PIF_Ram[CurPos] & 0xC0) == 0) {
				if (Channel < 4) {
					ProcessControllerCommand(Channel,(int8_t*)&PIF_Ram[CurPos]);
				}
				CurPos += PIF_Ram[CurPos] + (PIF_Ram[CurPos + 1] & 0x3F) + 1;
				Channel += 1;
			} else
...
void Cpu::ProcessControllerCommand ( int32_t Control, int8_t * Command) {
	Command[1] |= 0x80;
}
