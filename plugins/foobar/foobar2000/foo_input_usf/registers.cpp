...
void Cpu::SetupRegisters(N64_REGISTERS * n64_Registers) {
...
void Recompiler::ChangeFPURegFormat (BLOCK_SECTION * Section, int32_t Reg, int32_t OldFormat, int32_t NewFormat, int32_t RoundingModel) {
	int32_t i;
...
void COMPILER_CALL Recompiler::ChangeMiIntrMask (uint32_t mask) {
	mask = RegModValue;
	if ( ( mask & MI_INTR_MASK_CLR_SP ) != 0 ) { MI_INTR_MASK_REG &= ~MI_INTR_MASK_SP; }
	if ( ( mask & MI_INTR_MASK_SET_SP ) != 0 ) { MI_INTR_MASK_REG |= MI_INTR_MASK_SP; }
	if ( ( mask & MI_INTR_MASK_CLR_SI ) != 0 ) { MI_INTR_MASK_REG &= ~MI_INTR_MASK_SI; }
	if ( ( mask & MI_INTR_MASK_SET_SI ) != 0 ) { MI_INTR_MASK_REG |= MI_INTR_MASK_SI; }
	if ( ( mask & MI_INTR_MASK_CLR_AI ) != 0 ) { MI_INTR_MASK_REG &= ~MI_INTR_MASK_AI; }
	if ( ( mask & MI_INTR_MASK_SET_AI ) != 0 ) { MI_INTR_MASK_REG |= MI_INTR_MASK_AI; }
	if ( ( mask & MI_INTR_MASK_CLR_VI ) != 0 ) { MI_INTR_MASK_REG &= ~MI_INTR_MASK_VI; }
	if ( ( mask & MI_INTR_MASK_SET_VI ) != 0 ) { MI_INTR_MASK_REG |= MI_INTR_MASK_VI; }
	if ( ( mask & MI_INTR_MASK_CLR_PI ) != 0 ) { MI_INTR_MASK_REG &= ~MI_INTR_MASK_PI; }
	if ( ( mask & MI_INTR_MASK_SET_PI ) != 0 ) { MI_INTR_MASK_REG |= MI_INTR_MASK_PI; }
	if ( ( mask & MI_INTR_MASK_CLR_DP ) != 0 ) { MI_INTR_MASK_REG &= ~MI_INTR_MASK_DP; }
	if ( ( mask & MI_INTR_MASK_SET_DP ) != 0 ) { MI_INTR_MASK_REG |= MI_INTR_MASK_DP; }
}

void Recompiler::ChangeMiModeReg (void) {
	MI_MODE_REG &= ~0x7F;
...
void COMPILER_CALL Recompiler::ChangeSpStatus (uint32_t astatus)
{
//	_asm int 3
	uint32_t SpStatus ;
	SpStatus = astatus;
	if ( ( SpStatus & SP_CLR_HALT ) != 0) { SP_STATUS_REG &= ~SP_STATUS_HALT; }
	if ( ( SpStatus & SP_SET_HALT ) != 0) { SP_STATUS_REG |= SP_STATUS_HALT;  }
	if ( ( SpStatus & SP_CLR_BROKE ) != 0) { SP_STATUS_REG &= ~SP_STATUS_BROKE; }
	if ( ( SpStatus & SP_CLR_INTR ) != 0) {
		MI_INTR_REG &= ~MI_INTR_SP;
		CheckInterrupts();
	}

	if ( ( SpStatus & SP_CLR_SSTEP ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SSTEP; }
	if ( ( SpStatus & SP_SET_SSTEP ) != 0) { SP_STATUS_REG |= SP_STATUS_SSTEP;  }
	if ( ( SpStatus & SP_CLR_INTR_BREAK ) != 0) { SP_STATUS_REG &= ~SP_STATUS_INTR_BREAK; }
	if ( ( SpStatus & SP_SET_INTR_BREAK ) != 0) { SP_STATUS_REG |= SP_STATUS_INTR_BREAK;  }
	if ( ( SpStatus & SP_CLR_SIG0 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG0; }
	if ( ( SpStatus & SP_SET_SIG0 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG0;  }
	if ( ( SpStatus & SP_CLR_SIG1 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG1; }
	if ( ( SpStatus & SP_SET_SIG1 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG1;  }
	if ( ( SpStatus & SP_CLR_SIG2 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG2; }
	if ( ( SpStatus & SP_SET_SIG2 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG2;  }
	if ( ( SpStatus & SP_CLR_SIG3 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG3; }
	if ( ( SpStatus & SP_SET_SIG3 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG3;  }
	if ( ( SpStatus & SP_CLR_SIG4 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG4; }
	if ( ( SpStatus & SP_SET_SIG4 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG4;  }
	if ( ( SpStatus & SP_CLR_SIG5 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG5; }
	if ( ( SpStatus & SP_SET_SIG5 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG5;  }
	if ( ( SpStatus & SP_CLR_SIG6 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG6; }
	if ( ( SpStatus & SP_SET_SIG6 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG6;  }
	if ( ( SpStatus & SP_CLR_SIG7 ) != 0) { SP_STATUS_REG &= ~SP_STATUS_SIG7; }
	if ( ( SpStatus & SP_SET_SIG7 ) != 0) { SP_STATUS_REG |= SP_STATUS_SIG7;  }

	RunRsp();

}

int32_t Recompiler::Free8BitX86Reg (BLOCK_SECTION * Section) {
	int32_t x86Reg, count, MapCount[X86_REGS], MapReg[X86_REGS];
...
int32_t Recompiler::FreeX86Reg (BLOCK_SECTION * Section) {
	int32_t x86Reg, count, MapCount[X86_REGS], MapReg[X86_REGS], StackReg;
...
uint32_t Recompiler::Is8BitReg (int32_t x86Reg) {
...
void Recompiler::Load_FPR_ToTop (BLOCK_SECTION * Section, int32_t Reg, int32_t RegToLoad, int32_t Format) {
	int32_t i;
	...
	if (RegInStack(Section,RegToLoad,Format)) {
		if (Reg != RegToLoad) {
			if (FpuMappedTo((StackTopPos - 1) & 7) != (uint32_t)RegToLoad) {
				...
				fpuLoadReg((int32_t *)StackTopPos,StackPosition(Section,RegToLoad));
		...
		case FPU_Dword:
			...
			fpuLoadIntegerDwordFromX86Reg((int32_t *)&StackTopPos,TempReg);
			break;
		case FPU_Qword:
			MovePointerToX86reg(&FPRDoubleLocation[RegToLoad],TempReg);
			fpuLoadIntegerQwordFromX86Reg((int32_t *)&StackTopPos,TempReg);
			break;
		case FPU_Float:
			MovePointerToX86reg(&FPRFloatLocation[RegToLoad],TempReg);
...
void Recompiler::Map_GPR_32bit (BLOCK_SECTION * Section, int32_t Reg, uint32_t SignValue, int32_t MipsRegToLoad) {
	int x86Reg,count;
...
void Recompiler::Map_GPR_64bit (BLOCK_SECTION * Section, int32_t Reg, int32_t MipsRegToLoad) {
...
int32_t Recompiler::Map_TempReg (BLOCK_SECTION * Section, int32_t x86Reg, int32_t MipsReg, uint32_t LoadHiWord) {
...
void Recompiler::ProtectGPR(BLOCK_SECTION * Section, uint32_t Reg) {
...
uint32_t Recompiler::RegInStack(BLOCK_SECTION * Section,int32_t Reg, int32_t Format) {
	int i;
...
int32_t Recompiler::StackPosition (BLOCK_SECTION * Section, int32_t Reg) {
	int32_t i;
...
int Recompiler::UnMap_8BitTempReg (BLOCK_SECTION * Section) {
	int32_t count;
...
void Recompiler::UnMap_AllFPRs ( BLOCK_SECTION * Section ) {
	int32_t StackPos;
	...
	for (;;) {
		...
		for (i = 0; i < 8; i++) {
			if (FpuMappedTo((StartPos + i) & 7) != -1 ) { fpuIncStack((int32_t *)&StackTopPos); }
		}
...
void Recompiler::UnMap_FPR (BLOCK_SECTION * Section, int32_t Reg, int32_t WriteBackValue ) {
	int32_t TempReg;
	int32_t i;
	...
	for (i = 0; i < 8; i++) {
		...
		if (WriteBackValue) {
			...
			case FPU_Dword:
				...
				fpuStoreIntegerDwordFromX86Reg((int32_t *)&StackTopPos,TempReg, 1);
				break;
			case FPU_Qword:
				MovePointerToX86reg(&FPRDoubleLocation[FpuMappedTo(StackTopPos)],TempReg);
				fpuStoreIntegerQwordFromX86Reg((int32_t *)&StackTopPos,TempReg, 1);
				break;
			case FPU_Float:
				MovePointerToX86reg(&FPRFloatLocation[FpuMappedTo(StackTopPos)],TempReg);
				fpuStoreDwordFromX86Reg((int32_t *)&StackTopPos,TempReg, 1);
				break;
			case FPU_Double:
				MovePointerToX86reg(&FPRDoubleLocation[FpuMappedTo(StackTopPos)],TempReg);
				fpuStoreQwordFromX86Reg((int32_t *)&StackTopPos,TempReg, 1);
				break;
			}
			x86Protected(TempReg) = 0;
...
void Recompiler::UnMap_GPR (BLOCK_SECTION * Section, uint32_t Reg, int32_t WriteBackValue) {
	if (Reg == 0) {
		return;
	}
	if (IsUnknown(Reg)) { return; }
...
int Recompiler::UnMap_TempReg (BLOCK_SECTION * Section) {
	int count;
...
int Recompiler::UnMap_TempRegSet (REG_INFO * RegWorking) {
	int count;
...
uint32_t Recompiler::UnMap_X86reg (BLOCK_SECTION * Section, uint32_t x86Reg) {
	int count;
...
void Recompiler::UnProtectGPR(BLOCK_SECTION * Section, uint32_t Reg) {
	if (IsUnknown(Reg)) { return; }
	if (IsConst(Reg)) { return; }
	if (Is64Bit(Reg)) {
		x86Protected(MipsRegHi(Reg)) = 0;
...
void Cpu::UpdateCurrentHalfLine (void)
{
	if (Timers->Timer < 0) {
		HalfLine = 0;
		return;
...
void Recompiler::WriteBackRegisters (BLOCK_SECTION * Section) {
	int32_t count;
	uint32_t bEdiZero = 0;
	uint32_t bEsiSign = 0;
	/*** coming soon ***/
	...
	for (count = 1; count < X86_REGS; count ++) { UnMap_X86reg (Section, count); }
