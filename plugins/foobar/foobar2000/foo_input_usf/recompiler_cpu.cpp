...
static BLOCK_SECTION * ExistingSection(BLOCK_SECTION * StartSection, uint32_t Addr, uint32_t Test);
static void InitilizeRegSet(REG_INFO * RegSet);
static void InheritConstants(BLOCK_SECTION * Section);
static uint32_t IsAllParentLoops(BLOCK_SECTION * Section, BLOCK_SECTION * Parent, uint32_t IgnoreIfCompiled, uint32_t Test);
static void InitilzeSection (BLOCK_SECTION * Section, BLOCK_SECTION * Parent, uint32_t StartAddr, uint32_t ID);

Recompiler::Recompiler(Usf & mUsf) : Cpu(mUsf)
{
	LastTest = 0;
	FPU_RoundingMode = 0x0000;
	BranchCompare = 0;

	LastTest = EffectDelaySlot = DoneJumpDelay = DoneContinueDelay = FPU_RoundingMode = BranchCompare = 0;

	memset(&RegBeforeDelay, 0, sizeof(REG_INFO));
...
static void AddParent(BLOCK_SECTION * Section, BLOCK_SECTION * Parent)
{
	int32_t NoOfParents = 0, count = 0;
...
void Recompiler::AnalyseBlock (void) {
	int Changed;
...
static int ConstantsType (int64_t Value) {
...
void (* Recompiler::Compiler4300iBlock(void))() {
	...
	return (void (*)()) BlockInfo.CompiledLocation;
}


void (* Recompiler::CompileDelaySlot(void))() {
...
void CallResetRecompCode(Recompiler * recomp)
{
	recomp->ResetRecompCode();
}

void Recompiler::CompileExit (uint32_t TargetPC, REG_INFO * ExitRegSet, int reason, int CompileNow, void (Recompiler::*x86Jmp)(uint32_t Value)) {
...
#else
				MoveVariableToX86reg((uint8_t *)JumpTable + pAddr,x86_ECX);
#endif
				Jump2 = NULL;
	...
	case DoSysCall:
		...
		exit(0);
		Pushad();
		CALL_CPUFUNC(DoSysCallException);
		Popad();
		Ret();
		break;
	case COP1_Unuseable:
		...
		Push(x86_EDX);
		Push(x86_ECX);
		CALL_CPUFUNC(DoCopUnusableException);
	...
	case ExitResetRecompCode:
		...
		Pushad();
		Push((int32_t)this);
		Call_Direct(CallResetRecompCode);
...
void Recompiler::CompileSystemCheck (uint32_t TimerModifier, uint32_t TargetPC, REG_INFO RegSet) {
	BLOCK_SECTION Section;
	uint8_t *Jump = NULL, *Jump2 = NULL;
	...
	Call_Direct(GetAddress(0, &Cpu::TimerDone));
...
void Recompiler::CreateSectionLinkage (BLOCK_SECTION * Section) {
...
void Recompiler::DetermineLoop(BLOCK_SECTION * Section, uint32_t Test, uint32_t Test2, uint32_t TestID) {
	if (Section == NULL) { return; }
...
void Recompiler::FillSectionInfo(BLOCK_SECTION * Section) {
	OPCODE Command;
...
void Recompiler::FixConstants (BLOCK_SECTION * Section, uint32_t Test, int * Changed) {
...
void Recompiler::FreeSection (BLOCK_SECTION * Section, BLOCK_SECTION * Parent) {
	if (Section == NULL) { return; }
	...
	if (Section->ParentSection) {
		...
		if (Section->ParentSection) {
			for (count = 0; count < NoOfParents; count++) {
				if (!IsAllParentLoops(Section,(BLOCK_SECTION*)Section->ParentSection[count],0,GetNewTestValue())) { return; }
			}
			for (count = 0; count < NoOfParents; count++) {
...
void Recompiler::GenerateSectionLinkage (BLOCK_SECTION * Section) {
		...
		if (TargetSection[count]->CompiledLocation != NULL) {
			...
			if (JumpInfo[count]->TargetPC <= Section->CompilePC) {
				...
				if (JumpInfo[count]->PermLoop) {
					if((JumpInfo[count]->TargetPC > 0x04300000) && (JumpInfo[count]->TargetPC < 0x053011cc)) _asm int 3
					MoveConstToVariable(JumpInfo[count]->TargetPC,&PROGRAM_COUNTER);
					Pushad();
					CALL_CPUFUNC(InPermLoop);
					Popad();
...
uint32_t Recompiler::GenerateX86Code (BLOCK_SECTION * Section, uint32_t Test) {
	int count;

//	_control87(_EM_INVALID, _MCW_EM);
//fprintf(fil7, "Recompile!\n");

	if (Section == NULL) { return 0; }
	if (Section->CompiledLocation != NULL) {
		if (Section->Test == Test) { return 0; }
		Section->Test = Test;
		if (GenerateX86Code((BLOCK_SECTION *)Section->ContinueSection,Test)) { return 1; }
		if (GenerateX86Code((BLOCK_SECTION *)Section->JumpSection,Test)) { return 1; }
		return 0;
	}
	if (Section->ParentSection) {
	...
	do {
		...
		case R4300i_REGIMM:
			switch (Opcode.rt) {
			case R4300i_REGIMM_BLTZ:Compile_R4300i_Branch(Section,&Recompiler::BLTZ_Compare,BranchTypeRs, 0); break;
			case R4300i_REGIMM_BGEZ:Compile_R4300i_Branch(Section,&Recompiler::BGEZ_Compare,BranchTypeRs, 0); break;
			case R4300i_REGIMM_BLTZL:Compile_R4300i_BranchLikely(Section,&Recompiler::BLTZ_Compare, 0); break;
			case R4300i_REGIMM_BGEZL:Compile_R4300i_BranchLikely(Section,&Recompiler::BGEZ_Compare, 0); break;
			case R4300i_REGIMM_BLTZAL:Compile_R4300i_Branch(Section,&Recompiler::BLTZ_Compare,BranchTypeRs, 1); break;
			case R4300i_REGIMM_BGEZAL:Compile_R4300i_Branch(Section,&Recompiler::BGEZ_Compare,BranchTypeRs, 1); break;
			default:
				Compile_R4300i_UnknownOpcode(Section); break;
			}
			break;
		case R4300i_BEQ: Compile_R4300i_Branch(Section,&Recompiler::BEQ_Compare,BranchTypeRsRt,0); break;
		case R4300i_BNE: Compile_R4300i_Branch(Section,&Recompiler::BNE_Compare,BranchTypeRsRt,0); break;
		case R4300i_BGTZ:Compile_R4300i_Branch(Section,&Recompiler::BGTZ_Compare,BranchTypeRs,0); break;
		case R4300i_BLEZ:Compile_R4300i_Branch(Section,&Recompiler::BLEZ_Compare,BranchTypeRs,0); break;
		...
		case R4300i_CP1:
			...
			case R4300i_COP1_BC:
				switch (Opcode.ft) {
				case R4300i_COP1_BC_BCF: Compile_R4300i_Branch(Section,&Recompiler::COP1_BCF_Compare,BranchTypeCop1,0); break;
				case R4300i_COP1_BC_BCT: Compile_R4300i_Branch(Section,&Recompiler::COP1_BCT_Compare,BranchTypeCop1,0); break;
				case R4300i_COP1_BC_BCFL: Compile_R4300i_BranchLikely(Section,&Recompiler::COP1_BCF_Compare,0); break;
				case R4300i_COP1_BC_BCTL: Compile_R4300i_BranchLikely(Section,&Recompiler::COP1_BCT_Compare,0); break;
				default:
					Compile_R4300i_UnknownOpcode(Section); break;
				}
				break;
			...
			break;
		case R4300i_BEQL: Compile_R4300i_BranchLikely(Section,&Recompiler::BEQ_Compare,0); break;
		case R4300i_BNEL: Compile_R4300i_BranchLikely(Section,&Recompiler::BNE_Compare,0); break;
		case R4300i_BGTZL:Compile_R4300i_BranchLikely(Section,&Recompiler::BGTZ_Compare,0); break;
		case R4300i_BLEZL:Compile_R4300i_BranchLikely(Section,&Recompiler::BLEZ_Compare,0); break;
		...
		{
			//UnMap_GPR2 (Section, count, 1);
			//if(x86Mapped(x86Reg) == GPR_Mapped) {
			//	uint32_t Reg = MipsRegLo(Reg)
			//}
			if(IsMapped(count)) {
				MoveX86regToVariable(MipsRegLo(count),&GPR[count].UW[0]);
				if(Is64Bit(count)) {
					if(x86Mapped(MipsRegHi(count)) == NotMapped)
						continue;
					MoveX86regToVariable(MipsRegHi(count),&GPR[count].UW[1]);
				}
			}
		}*/
...
uint32_t Recompiler::GetNewTestValue(void) {
...
static void InitilizeRegSet(REG_INFO * RegSet) {
	...
	for (count = 0; count < X86_REGS; count ++ ) {
...
uint32_t Recompiler::InheritParentInfo (BLOCK_SECTION * Section) {
	...
	for (count = 0;count < NoOfParents;count++) {
		...
		for (count2 = 1; count2 < 32; count2++) {
			...
			for (count3 = 1; count3 < X86_REGS; count3 ++) { x86Protected(count3) = 0; }
		}
	}
	memcpy(&Section->RegStart,&Section->RegWorking,sizeof(REG_INFO));
...
static uint32_t IsAllParentLoops(BLOCK_SECTION * Section, BLOCK_SECTION * Parent, uint32_t IgnoreIfCompiled, uint32_t Test) {
	...
	for (count = 0;Parent->ParentSection[count] != NULL;count++) {
		if (!IsAllParentLoops(Section,(BLOCK_SECTION *) Parent->ParentSection[count],IgnoreIfCompiled,Test)) { return 0; }
	}
	return 1;
}
...
void Recompiler::MarkCodeBlock (uint32_t PAddr) {
	if (PAddr < RdramSize) {
...
void Recompiler::StartCPU (void ) {
	uintptr_t Addr;
	void (*Block)(void) = 0;

	InitRecompilerMemory();
...
void * Recompiler::RunCpu(uint32_t * Bytes)
{
	uintptr_t Addr;
	void (*Block)(void) = 0;
...
void Recompiler::SyncRegState (BLOCK_SECTION * Section, REG_INFO * SyncTo) {
