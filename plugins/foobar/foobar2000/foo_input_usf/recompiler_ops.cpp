...
void Recompiler::CompileReadTLBMiss (BLOCK_SECTION * Section, int32_t AddressReg, int32_t LookUpReg ) {
#ifdef USEX64
	...
	CompileExit(Section->CompilePC,&Section->RegWorking,TLBReadMiss,0,&Recompiler::JeLabel32);
	//CompileExit(Section->CompilePC,&Section->RegWorking,TLBReadMiss,0,(void (*)(uint32_t))-1);
}

/************************** Branch functions  ************************/
void Recompiler::Compile_R4300i_Branch (BLOCK_SECTION * Section, void (Recompiler::*CompareFunc)(BLOCK_SECTION * Section), int32_t BranchType, uint32_t Link) {
//void Recompiler::Compile_R4300i_Branch (BLOCK_SECTION * Section, void (*CompareFunc)(BLOCK_SECTION * Section), int32_t BranchType, uint32_t Link) {
	int32_t count;
	...
	if ( NextInstruction == NORMAL ) {
		...
		if (EffectDelaySlot) {
			...
			for (count = 1; count < X86_REGS; count ++) { x86Protected(count) = 0; }
...
void Recompiler::Compile_R4300i_BranchLikely (BLOCK_SECTION * Section, void (Recompiler::*CompareFunc)(BLOCK_SECTION * Section), uint32_t Link) {
	int32_t count;
...
void Recompiler::BNE_Compare (BLOCK_SECTION * Section) {
	uint8_t *Jump;
...
void Recompiler::BEQ_Compare (BLOCK_SECTION * Section) {
	uint8_t *Jump;
...
void Recompiler::BGTZ_Compare (BLOCK_SECTION * Section) {
...
void Recompiler::BLEZ_Compare (BLOCK_SECTION * Section) {
	if (IsConst(Opcode.rs)) {
		if (Is64Bit(Opcode.rs)) {
			if (MipsReg_S(Opcode.rs) <= 0) {
				Section->Jump.FallThrough = 1;
...
void Recompiler::BLTZ_Compare (BLOCK_SECTION * Section) {
	if (IsConst(Opcode.rs)) {
		if (Is64Bit(Opcode.rs)) {
			if (MipsReg_S(Opcode.rs) < 0) {
				Section->Jump.FallThrough = 1;
...
void Recompiler::BGEZ_Compare (BLOCK_SECTION * Section) {
	if (IsConst(Opcode.rs)) {
		if (Is64Bit(Opcode.rs)) {
...
void Recompiler::COP1_BCF_Compare (BLOCK_SECTION * Section) {
...
void Recompiler::COP1_BCT_Compare (BLOCK_SECTION * Section) {
	TestVariable(FPCSR_C,&FPCR[31]);
	if (Section->Cont.FallThrough) {
		JneLabel32 (  0 );
		Section->Jump.LinkLocation = RecompPos - 4;
...
/*************************  OpCode functions *************************/
void Recompiler::Compile_R4300i_J (BLOCK_SECTION * Section) {
	if ( NextInstruction == NORMAL ) {
...
void Recompiler::Compile_R4300i_JAL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_ADDI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_ADDIU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SLTIU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SLTI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_ANDI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_ORI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_XORI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_LUI (BLOCK_SECTION * Section) {
...
void Recompiler::r4300i_DADDIU (void) {
	GPR[Opcode.rt].DW = GPR[Opcode.rs].DW + (int64_t)((short)Opcode.immediate);
}


void Recompiler::Compile_R4300i_DADDIU (BLOCK_SECTION * Section) {
	...
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_DADDIU));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();
}

void Recompiler::r4300i_LDL2() {
...
void Recompiler::Compile_R4300i_LDL (BLOCK_SECTION * Section) {
	...
	Pushad();
#ifdef USEX64
	MoveConstQwordToX86reg((uintptr_t)this, x86_ECX );
	Call_Direct(r4300i_LDL2);
#else
	Pushad();
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_LDL2));
	AdcConstToX86Reg(x86_ESP, 4);
	Popad();
#endif
	Popad();
...
void Recompiler::Compile_R4300i_LDR (BLOCK_SECTION * Section) {
...
#else
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_LDR2));
	AdcConstToX86Reg(x86_ESP, 4);
#endif
	Popad();
}
...
void Recompiler::Compile_R4300i_LB (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_LH (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_LWL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_LW (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_LBU (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_LHU (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_LWR (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2, Offset, shift;
...
void Recompiler::Compile_R4300i_LWU (BLOCK_SECTION * Section) {			//added by Witten
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SB (BLOCK_SECTION * Section){
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SH (BLOCK_SECTION * Section){
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SWL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SW (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SWR (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2, Value, Offset, shift;
...
void Recompiler::r4300i_SDL (void) {
...
void Recompiler::Compile_R4300i_SDL (BLOCK_SECTION * Section) {
	...
	MoveConstToVariable(Opcode.Hex, &Opcode.Hex );
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_SDL));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();

}

void Recompiler::r4300i_SDR (void) {
	uint32_t Offset, Address;
	uint64_t Value;
...
void Recompiler::Compile_R4300i_SDR (BLOCK_SECTION * Section) {
	...
	MoveConstToVariable(Opcode.Hex, &Opcode.Hex );
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_SDR));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();

}

void Recompiler::Compile_R4300i_CACHE (BLOCK_SECTION * Section){
/*
...
void Recompiler::Compile_R4300i_LL (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SC (BLOCK_SECTION * Section){
	uint32_t TempReg1, TempReg2;
	uint8_t * Jump;
...
void Recompiler::Compile_R4300i_LD (BLOCK_SECTION * Section) {
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SD (BLOCK_SECTION * Section){
	uint32_t TempReg1, TempReg2;
...
void Recompiler::Compile_R4300i_SPECIAL_SLL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SRL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SRA (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SLLV (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SRLV (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SRAV (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_JR (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_JALR (BLOCK_SECTION * Section) {
	if ( NextInstruction == NORMAL ) {
...
void Recompiler::Compile_R4300i_SPECIAL_SYSCALL (BLOCK_SECTION * Section) {
	CompileExit(Section->CompilePC,&Section->RegWorking,DoSysCall,1,NULL);
}

void Recompiler::Compile_R4300i_SPECIAL_BREAK (BLOCK_SECTION * Section) {
	//CompileExit(Section->CompilePC,Section->RegWorking,DoBreak,1,NULL);
	MoveConstToVariable(1,WaitMode);
}


void Recompiler::Compile_R4300i_SPECIAL_MFLO (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_MTLO (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_MFHI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_MTHI (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSLLV (BLOCK_SECTION * Section) {
	uint8_t * Jump[2];
...
void Recompiler::Compile_R4300i_SPECIAL_DSRLV (BLOCK_SECTION * Section) {
	uint8_t * Jump[2];
...
void Recompiler::Compile_R4300i_SPECIAL_DSRAV (BLOCK_SECTION * Section) {
	uint8_t * Jump[2];
...
void Recompiler::Compile_R4300i_SPECIAL_MULT ( BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_MULTU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DIV (BLOCK_SECTION * Section) {
	uint8_t *Jump[2];
...
void Recompiler::Compile_R4300i_SPECIAL_DIVU ( BLOCK_SECTION * Section) {
	uint8_t *Jump[2];
	int32_t x86reg;
...
void Recompiler::r4300i_SPECIAL_DMULT (void) {
	MIPS_DWORD Tmp[3];
...
void Recompiler::Compile_R4300i_SPECIAL_DMULT (BLOCK_SECTION * Section) {
	...
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_SPECIAL_DMULT));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();
}

void Recompiler::Compile_R4300i_SPECIAL_DMULTU (BLOCK_SECTION * Section) {
...
void Recompiler::r4300i_SPECIAL_DDIV (void) {
...
void Recompiler::r4300i_SPECIAL_DDIVU (void) {
	if ( GPR[Opcode.rt].UDW != 0 ) {
...
void Recompiler::Compile_R4300i_SPECIAL_DDIV (BLOCK_SECTION * Section) {
	...
	MoveConstToVariable(Opcode.Hex, &Opcode.Hex );
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_SPECIAL_DDIV));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();
}

void Recompiler::Compile_R4300i_SPECIAL_DDIVU (BLOCK_SECTION * Section) {
	...
	MoveConstToVariable(Opcode.Hex, &Opcode.Hex );
	PushImm32((uintptr_t)this);
	Call_Direct(GetAddress(0, &Recompiler::r4300i_SPECIAL_DDIVU));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();
}

void Recompiler::Compile_R4300i_SPECIAL_ADD (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_ADDU (BLOCK_SECTION * Section) {
	int32_t source1 = Opcode.rd == Opcode.rt?Opcode.rt:Opcode.rs;
	int32_t source2 = Opcode.rd == Opcode.rt?Opcode.rs:Opcode.rt;
...
void Recompiler::Compile_R4300i_SPECIAL_SUB (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SUBU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_AND (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_OR (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_XOR (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_NOR (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SLT (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_SLTU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DADD (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DADDU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSUB (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSUBU (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSLL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSRL (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSRA (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSLL32 (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSRL32 (BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_SPECIAL_DSRA32 (BLOCK_SECTION * Section) {
...
/************************** COP0 functions **************************/
void Recompiler::Compile_R4300i_COP0_MF(BLOCK_SECTION * Section) {
...
void Recompiler::Compile_R4300i_COP0_MT (BLOCK_SECTION * Section) {
	...
	case 30: //ErrEPC
		...
		case 9: //Count
			...
			Call_Direct(GetAddress(0, &Cpu::ChangeCompareTimer));
			AddConstToX86Reg(x86_ESP, 4);
			Popad();
			break;
		case 11: //Compare
	...
	case 12: //Status
		...
		Jump = RecompPos - 1;
		Pushad();
		CALL_CPUFUNC(SetFpuLocations);
		Popad();
		*(char *)(Jump)= (char )(((char )(RecompPos)) - (((char )(Jump)) + 1));
...
/************************** COP0 CO functions ***********************/
void Recompiler::Compile_R4300i_COP0_CO_TLBR( BLOCK_SECTION * Section) {
	Pushad();
	PushImm32((uint32_t)dynamic_cast<Cpu*>(this));
	Call_Direct(GetAddress(0, &Cpu::TLB_Read));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();
}

void Recompiler::Compile_R4300i_COP0_CO_TLBWI( BLOCK_SECTION * Section) {
	Pushad();
	...
	Push(x86_ECX);
	PushImm32((uint32_t)dynamic_cast<Cpu*>(this));
	Call_Direct(GetAddress(0, &Cpu::WriteTLBEntry));
	AddConstToX86Reg(x86_ESP, 8);
#endif
	Popad();
}

void Recompiler::Compile_R4300i_COP0_CO_TLBWR( BLOCK_SECTION * Section) {
	if (BlockRandomModifier != 0) { SubConstFromVariable(BlockRandomModifier,&CP0[1]); }
	BlockRandomModifier = 0;
	Pushad();
...
void Recompiler::Compile_R4300i_COP0_CO_TLBP( BLOCK_SECTION * Section) {
	Pushad();
	PushImm32((uint32_t)dynamic_cast<Cpu*>(this));
	Call_Direct(GetAddress(0, &Cpu::TLB_Probe));
	AddConstToX86Reg(x86_ESP, 4);
	Popad();
}

void Recompiler::compiler_COP0_CO_ERET (void) {
...
void Recompiler::Compile_R4300i_COP0_CO_ERET( BLOCK_SECTION * Section) {
	uint8_t * Jump, * Jump2;
	WriteBackRegisters(Section);
	Pushad();

	MoveVariableToX86reg(&STATUS_REGISTER, x86_EAX);
	AndConstToX86Reg(x86_EAX, STATUS_ERL);
	CompConstToX86reg(x86_EAX, 0);
	JeLabel8(0);
	Jump = RecompPos - 1;

	MoveVariableToX86reg(&ERROREPC_REGISTER, x86_EAX);
	MoveX86regToVariable(x86_EAX, &PROGRAM_COUNTER);
	AndConstToVariable(~STATUS_ERL, &STATUS_REGISTER);
	JmpLabel8(0);
	Jump2 = RecompPos - 1;

	SetJump8(Jump, RecompPos);

	MoveVariableToX86reg(&EPC_REGISTER, x86_EAX);
	MoveX86regToVariable(x86_EAX, &PROGRAM_COUNTER);
	AndConstToVariable(~STATUS_EXL, &STATUS_REGISTER);
...
/************************** Other functions **************************/
void Recompiler::Compile_R4300i_UnknownOpcode (BLOCK_SECTION * Section) {
...
//	Int3();
	...
	//PushImm32((uintptr_t)this);
	//Call_Direct(GetAddress(0, &Recompiler::R4300i_UnknownOpcode));
	BreakPoint();
	//AddConstToX86Reg(x86_ESP, 4);
	Popad();
	Ret();
