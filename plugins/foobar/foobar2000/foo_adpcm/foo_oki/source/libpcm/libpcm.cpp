...
#endif
#ifndef LIBPCM_DISABLE_DRACULAX_SUPPORT
#define LIBPCM_ENABLE_DRACULAX_ORIGINALTABLE
#ifdef LIBPCM_ENABLE_DRACULAX_ORIGINALTABLE
static const uint_t dvi_dlt_tbl[89*8] =
{
...
/*
	...
	This table results in slightly inaccurate
	output, which causes a gradual DC offset
	to build up in the output.
*/
...
#else
	delta = dvi_dlt_tbl[(d->adp_scale[ch] << 3) + (v & 7)];
#endif
	d->adp_scale[ch] += oki_scale_tbl[v & 7];
