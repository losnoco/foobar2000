...
virtual void insert_extension (class window * p_ext, unsigned height, unsigned width)=0;
...
public:
	 ...
	 *
	 * \see window_flag::size_limit_changed
	 */
	virtual void on_size_limit_change(HWND wnd, unsigned flags)=0;
	...
	* \return   combination of window_flag::RSZ_* to indicate whether the width or height can be modified
	*
	* \pre May only be called by a hosted UI extension.
	*
	* \see window_flag::resize_flags
	*/
	...
	* 
	* \param[in] wnd   handle to the window to test
	* \return   combination of window_flag::RSZ_* to indicate whether the width or height is being modified
	*
	* \todo finish
	*/
	 ...
	 * \param[in] wnd window handle of the extension's window
	 *
	 * \see window::init_or_take_ownership
	 */
	virtual void relinquish_ownership(HWND wnd)=0;
 ...
 */
template<class T>
class window_host_factory : public service_factory_t<window_host,T> {};
 ...
 */
template<class T>
class window_host_factory_single : public service_factory_single_t<window_host,T> {};
 ...
 */
template<class T>
class window_host_factory_transparent_single : public service_factory_single_transparent_t<window_host,T> {};
