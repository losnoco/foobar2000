...
int mpglib::decode( const unsigned char *in,int isize,sample *out,int osize,int *done)
{
	...
	if (isize>MAXFRAMESIZE) isize = MAXFRAMESIZE;
	...
	mp.header = getbits(32);;

	if (!decode_header(&mp.fr,mp.header)) return MP3_ERR;
