...
class mpglib
{
	...
	void III_get_side_info_1(struct III_sideinfo *si,int stereo,int ms_stereo,long sfreq);
	void III_get_side_info_2(struct III_sideinfo *si,int stereo,int ms_stereo,long sfreq);
	...
	int do_layer2( PMPSTR mp,sample *pcm_sample,int *pcm_point);
...
public:
	...
	int decode( const unsigned char *in,int isize,sample *out,int osize,int *done);
