...
int ebur128_calc_gating_block(ebur128_state* st, size_t frames_per_block,
  ...
  if (optional_output) {
    *optional_output = sum;
    ...
    block = (ebur128_dq_entry *) malloc(sizeof(struct ebur128_dq_entry));
    if (!block) return -1;
    block->z = sum;
...
double ebur128_gated_loudness(ebur128_state* st,
  ...
  if (SLIST_EMPTY(&st->block_list)) return std::numeric_limits<float>::quiet_NaN();
  ...
  if (!above_thresh_counter) return std::numeric_limits<float>::quiet_NaN();
...
double ebur128_energy_in_interval(ebur128_state* st, size_t interval_frames) {
  ...
  if (interval_frames > st->audio_data_frames) return std::numeric_limits<float>::infinity();
...
double ebur128_loudness_range(ebur128_state* st) {
  ...
  if (!stl_vector) return std::numeric_limits<float>::infinity();
  i = 0;
  SLIST_FOREACH(it, &st->short_term_block_list, entries) {
    stl_vector[i] = it->z;
    ++i;
