...
protected:
	...
	bool tag_write(const char *p, reader *r)
	{
		t_size l = strlen(p);
		array_t<char> lbuf;
		if (!lbuf.resize(unsigned(l))) return false;
		char *d = &lbuf[0];
	...
	bool tagsave(reader *r, const file_info & p_info)
	{
		...
		/* write play info */
		int icnt = p_info.info_get_count();
		...
		/* write meta data */
		int mcnt = p_info.meta_get_count();
	...
	bool xsf_reopen(reader *r)
	{
		...
		if (!m_filebuffer.resize(unsigned(m_filesize))) return false;
		return r->read(&m_filebuffer[0], unsigned(l)) == l;
	}
	...
	void xsf_seek(t_uint64 &seek_smp)
	{
		...
		if (bufsize < xsf_total_sample_width)
		{
			...
			m_buffer.resize(unsigned(bufsize * xsf_total_sample_width));
		}
		if (seek_smp < cur_smp)
		{
			xsf_restart();
		}
		while (unsigned(seek_smp) - unsigned(cur_smp) > bufsize)
		{
...
public:
	...
	{
		p_info.set_length(::MulDiv(int(end_smp), 1, xsf_sample_rate));
	...
	int get_samples_pcm(void ** out_buffer,int * out_size, int * srate, int * bps, int * nch)
	{
		...
		*out_size = int(bufsize * xsf_total_sample_width);
