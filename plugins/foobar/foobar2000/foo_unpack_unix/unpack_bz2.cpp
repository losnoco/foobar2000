...
static const char * retval_to_errmsg( int err )
{
	const char * str = 0;

	switch ( err )
	{
	//case RETVAL_LAST_BLOCK:    str = "Last block"; break;
	case RETVAL_NOT_BZIP_DATA: str = "Not BZIP2 data"; break;
	//case RETVAL_UNEXPECTED_INPUT_EOF: str = "Unexpected input EOF"; break;
	//case RETVAL_UNEXPECTED_OUTPUT_EOF: str = "Unexpected output EOF"; break;
	case RETVAL_DATA_ERROR:    str = "Data error"; break;
	//case RETVAL_OUT_OF_MEMORY: str = "Out of memory"; break;
	case RETVAL_OBSOLETE_INPUT: str = "Obsolete input"; break;
	}
...
struct bunzip_data {
	...
	bunzip_data()
	{
		writeCopies = writePos = writeRunCountdown = writeCount = writeCurrent = 0;
		inbufCount = inbufPos = 0;
		inbufBitCount = inbufBits = 0;
		memset( crc32Table, 0, sizeof( crc32Table ) );
		headerCRC = totalCRC = writeCRC = 0;
...
static unsigned int get_bits(bunzip_data *bd, char bits_wanted)
{
	...
	while (bd->inbufBitCount<bits_wanted) {
		...
		if(bd->inbufPos==bd->inbufCount) {
			if((bd->inbufCount = bd->in_fd->read(bd->inbuf, IOBUF_SIZE, *bd->abort)) <= 0)
...
static int get_next_block(bunzip_data *bd)
{
	...
	dbuf=bd->dbuf.get_ptr();
	dbufSize=bd->dbuf.get_size();
...
static int start_bunzip(bunzip_data **bdp, const service_ptr_t<file> & in_fd, abort_callback & p_abort)
{
	bunzip_data *bd;
	unsigned int i,j,c;
	...
	//if(!(bd=*bdp=)) return RETVAL_OUT_OF_MEMORY;
	bd = *bdp = new bunzip_data;
	...
	*/
	...
	bd->dbuf.set_size( 100000*(i-BZh0) );
...
{
	//char *outbuf;
	pfc::array_t<char> outbuf;
	bunzip_data *bd;
	int i;

	//if(!(outbuf=malloc(IOBUF_SIZE))) return RETVAL_OUT_OF_MEMORY;
	outbuf.set_size( IOBUF_SIZE );
	if(!(i=start_bunzip(&bd,src_fd,p_abort))) {
		for(;;) {
			if((i=read_bunzip(bd,outbuf.get_ptr(),IOBUF_SIZE)) <= 0) break;
			/*if(i!=write(dst_fd,outbuf,i)) {
				i=RETVAL_UNEXPECTED_OUTPUT_EOF;
				break;
			}*/
			dst_fd->write_object( outbuf.get_ptr(), i, p_abort );
		}
	}
	...
	if ( i < 0 ) throw exception_io_data( retval_to_errmsg( i ) );
}
...
class archive_bz2 : public archive_impl
{
public:
	...
	{
		if ( stricmp_utf8( pfc::string_extension( path ), "bz2" ) )
...
class unpacker_bz2 : public unpacker
{
public:
