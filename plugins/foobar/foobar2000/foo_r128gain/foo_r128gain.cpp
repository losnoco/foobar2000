...
static inline double rg_offset(double lu)
{
	...
	else return REFERENCE - lu;
}

static const GUID guid_r128_branch = { 0xdb7e74cb, 0xaa00, 0x482e, { 0xb6, 0xa2, 0x18, 0x57, 0x6d, 0xb6, 0x4b, 0x2a } };
...
{
	unsigned last_srate, last_channels;
...
class r128_scanner : public threaded_process_callback
{
	...
	struct job
	{
		...
		job( bool p_is_album, const metadb_handle_list & p_handles )
		{
			...
			m_handles = p_handles;
			m_tracks_total = m_tracks_left = p_handles.get_count();
			m_current_thread = 0;
			m_ctx = NULL;
			...
			try
			{
				...
				else
				{
					...
					service_ptr_t<dsp> m_resampler;
					{
						...
						m_track_index = m_current_job->m_tracks_total - m_current_job->m_handles.get_count();
						m_ctx = m_current_job->m_ctx;
					...
					catch (...)
					{
						...
						{
							insync( m_current_job->m_lock );
							if ( m_ctx ) m_current_job->m_ctx = m_ctx;
							m_current_job->m_thread_in_use[ m_current_thread ] = false;
...
DECLARE_COMPONENT_VERSION("EBU R128 Gain scanner", MY_VERSION,
	...
	"\n"
	"Copyright (C) 2012 Chris Moeller\n"
	"\n"
	"Portions copyright (c) 2012 Peter Belkner <pbelkner@snafu.de>\n"
	"\n"
...
VALIDATE_COMPONENT_FILENAME("foo_r128gain.dll");
