// modified from helpers/file_cached.h

template<unsigned blocksize>
class file_cached : public file {
public:
	static void g_create( service_ptr_t<file> & p_out, service_ptr_t<file> p_base, abort_callback & p_abort, t_filesize limit = infinite64 )
	{
		service_ptr_t<file_cached<blocksize> > temp;
		temp = new service_impl_t<file_cached<blocksize> >();
		temp->initialize( p_base, limit, p_abort );
		p_out = temp.get_ptr();
	}
private:
	void initialize( service_ptr_t< file > p_base, t_filesize limit, abort_callback & p_abort )
	{
		m_base = p_base;
		...
		m_position = m_position_base;

		m_size = m_base->get_size( p_abort );
		limit += m_position;
		if ( limit >= m_position && limit < m_size ) m_size = limit;
