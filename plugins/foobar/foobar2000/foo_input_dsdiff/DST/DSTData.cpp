...
int getbits(dstdata * data, long *outword, int out_bitptr);
...
int ResetReadingIndex(dstdata * data)
{
  ...
  data->m_BitCounter  = 0;
  data->m_BitPosition = -1;
  data->m_ByteCounter = 0;
  data->m_DataByte    = 0;
...
int CreateBuffer(dstdata * data, LONG Size)
{
  ...
  data->m_TotalBytes = Size;

  /* create new buffer for data */
  data->m_pDSTdata = (BYTE*) malloc (Size);

  if (data->m_pDSTdata == NULL)
  {
    data->m_TotalBytes = 0;
    hr = -1;
  }

  ResetReadingIndex( data );
...
int DeleteBuffer(dstdata * data)
{
  ...
  if (data->m_pDSTdata != NULL)
  {
    free( data->m_pDSTdata );
	data->m_pDSTdata = NULL;
  }
...
int FIO_BitGetChrUnsigned(dstdata * data, int Len, unsigned char *x)
{
  ...
  if (Len > 0)
  {
    return_value = getbits(data, &tmp, Len);
    *x = (unsigned char)tmp;
  }
  else if (Len == 0)
  {
  ...
  else
  {
    ...
    exit(1);*/
    throw std::exception( "a negative number of bits allocated" );
  }
  return return_value;
}
...
int getbits(dstdata * data, long *outword, int out_bitptr)
{
  data->m_BitCounter += out_bitptr;
  *outword = 0;
  while(--out_bitptr >= 0)
  {
    if (data->m_BitPosition < 0)
    {
      data->m_DataByte = data->m_pDSTdata[data->m_ByteCounter++];
      if (data->m_ByteCounter > data->m_TotalBytes)
      {
        return (-1); /* EOF */
      }
      data->m_BitPosition = 7;
    }
	if ((data->m_DataByte & (1 << data->m_BitPosition)/*data->m_mask[data->m_BitPosition]*/) != 0)
    {
      *outword |= 1 << out_bitptr; //data->m_mask[out_bitptr];
    }
    data->m_BitPosition--;
  }
