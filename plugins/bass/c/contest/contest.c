...
void main(int argc, char **argv)
{
	...
	DWORD flags=BASS_WASAPI_AUTOFORMAT|BASS_WASAPI_BUFFER|BASS_WASAPI_EXCLUSIVE; // default initialization flags
	BASS_WASAPI_DEVICEINFO di;
	...
	for (filep=1;filep<argc;filep++) {
		...
		else if (!strcmp(argv[filep],"-s")) flags&=~BASS_WASAPI_EXCLUSIVE;
	...
	if (filep==argc) {
		printf("\tusage: contest [-l] [-d #] [-s] [-e] <file>\n"
			"\t-l = list devices\n"
			"\t-d = device number\n"
			"\t-s = shared mode, else exclusive (if available)\n"
	...
	{ // setup output
		float buflen=(flags&BASS_WASAPI_EVENT?0.1:0.4);// using a smaller buffer with event-driven system (only affects exclusive mode)
		BASS_WASAPI_INFO wi;
		BASS_CHANNELINFO ci;
