	...
	switch (m) {
		...
		case WM_HSCROLL:
			if (l) {
				...
				BASS_SetConfig(BASS_CONFIG_BUFFER,10+info.minbuf+p); // update buffer length
				// recreate the MIDI stream with new buffer length
				BASS_StreamFree(stream);
				stream=BASS_MIDI_StreamCreate(17,MESS(32,BM_GETCHECK,0,0)?BASS_MIDI_SINCINTER:0,1);
				BASS_ChannelSetAttribute(stream,BASS_ATTRIB_MIDI_CPU,75);
				...
				if (drums) BASS_MIDI_StreamEvent(stream,16,MIDI_EVENT_DRUMS,drums); // set drum switch
				BASS_MIDI_StreamEvent(stream,16,MIDI_EVENT_PROGRAM,preset); // set program/preset
				{ // re-apply effects
					int a;
					for (a=0;a<5;a++)
			...
			if ((msg.message==WM_KEYDOWN && !(msg.lParam&0x40000000)) || msg.message==WM_KEYUP) {
				...
				for (key=0;key<KEYS;key++) {
					if (msg.wParam==keys[key]) {
						BASS_MIDI_StreamEvent(stream,16,MIDI_EVENT_NOTE,MAKEWORD((drums?36:60)+key,msg.message==WM_KEYDOWN?100:0)); // send note on/off event
						break;
					}
				}
			}
