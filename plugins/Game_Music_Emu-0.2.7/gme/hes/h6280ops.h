...
#define CHECK_IRQ_LINES 										\
	...
	{															\
		if ( h6280->irq_state[0] != CLEAR_LINE &&				\
			 !(h6280->irq_mask & 0x2) )							\
		{														\
			DO_INTERRUPT(h6280, H6280_IRQ1_VEC);						\
		}														\
		else													\
		if ( h6280->irq_state[1] != CLEAR_LINE &&				\
			 !(h6280->irq_mask & 0x1) )							\
		{														\
			DO_INTERRUPT(h6280, H6280_IRQ2_VEC);						\
        }                                                       \
		else													\
		...
		{														\
			h6280->irq_state[2] = CLEAR_LINE;					\
 			DO_INTERRUPT(h6280, H6280_TIMER_VEC);						\
       }                                                       	\
    }

extern void io_page_w(void *, int address, int value);
extern int io_page_r(void *, int address);

#define cpu_readop21_fast(addr)         h6280->read_ptr[(addr) >> 13][(addr) & 0x1FFF]
//#define cpu_readmem21_fast(addr)        (h6280->read_ptr[(addr) >> 13] == 0) ? io_page_r(h6280->private, (addr) & 0x1FFF) : h6280->read_ptr[(addr) >> 13][(addr) & 0x1FFF]
#define cpu_readmem21_fast(addr)	(h6280->mmr[(addr)>>13] == (0xFF<<13)) ? io_page_r(h6280->private, (addr) & 0x1FFF) : h6280->read_ptr[(addr) >> 13][(addr) & 0x1FFF]
#define cpu_writemem21_fast(addr,value) if(h6280->write_ptr[(addr) >> 13] == 0) io_page_w(h6280->private, (addr) & 0x1FFF, value); else (h6280->write_ptr[(addr) >> 13][(addr) & 0x1FFF] = value)

#define RDMEMZ(addr)        h6280->ram[addr & 0x1FFF];
#define WRMEMZ(addr,data)   h6280->ram[addr & 0x1FFF] = data;

#define RDZPWORD(addr)                              \
    ((addr&0xff)==0xff) ?                           \
        h6280->ram[(addr)&0x1fff]                          \
        +( h6280->ram[ ((addr-0xff)&0x1fff) ]    <<8) :    \
        h6280->ram[(addr)&0x1fff]                          \
        +( h6280->ram[ ((addr+1)&0x1fff)] <<8)
...
static inline void DO_INTERRUPT(h6280_Regs *h6280,unsigned int vector)
{
...
#define BSR 													\
	PUSH(PCH);													\
	PUSH(PCL);													\
	CYCTOCK(4); /* 4 cycles here, 4 in BRA */			\
	BRA(1)
...
{                                                               \
	...
	printf("SET: %02x\n",op);				\
        ...
        CYCTOCK(6);  /* Unsure of actual cycles used */  \
    }                                                           \
    P &= ~_fT;                                                  \
}                                                               
...
extern void vdc_w(void *, int address, int data);

#define ST0     vdc_w(h6280->private, 0, tmp)
#define ST1     vdc_w(h6280->private, 2, tmp)
#define ST2     vdc_w(h6280->private, 3, tmp)
...
extern void bank_set(void *, int bank, int value);
#define TAM                                                     \
    {                                                           \
        int shift;                                              \
        ...
        {                                                       \
            if(tmp & (1 << shift))                              \
            {                                                   \
                h6280->mmr[shift] = (A << 13);                   \
                bank_set(h6280->private, shift, A);                             \
            }                                                   \
 ...
 ***************************************************************/
#define TMA                                                     \
    if (tmp&0x01) A = (h6280->mmr[0] >> 13);                             \
    else                                                        \
    if (tmp&0x02) A = (h6280->mmr[1] >> 13);                             \
    else                                                        \
    if (tmp&0x04) A = (h6280->mmr[2] >> 13);                             \
    else                                                        \
    if (tmp&0x08) A = (h6280->mmr[3] >> 13);                             \
    else                                                        \
    if (tmp&0x10) A = (h6280->mmr[4] >> 13);                             \
    else                                                        \
    if (tmp&0x20) A = (h6280->mmr[5] >> 13);                             \
    else                                                        \
    if (tmp&0x40) A = (h6280->mmr[6] >> 13);                             \
    else                                                        \
    if (tmp&0x80) A = (h6280->mmr[7] >> 13)

/* 6280 ********************************************************
 * TRB  Test and reset bits
 ***************************************************************/
#define TRB                                                   	\
	tmp&=~A;P = (P & ~(_fN|_fV|_fT|_fZ))								\
		| ((tmp&0x80) ? _fN:0)									\
		| ((tmp&0x40) ? _fV:0)									\
		| ((tmp)  ? 0:_fZ);
