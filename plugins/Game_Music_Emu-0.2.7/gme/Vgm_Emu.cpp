...
void Vgm_Emu::mute_voices( int mask )
{
	...
	else
	{
		...
		psg.output( (mask & 0x80) ? NULL : &blip_buf );
		if ( ym2612.enabled() )
		{
			dac_synth.volume_unit( (mask & 0x40) ? 0.0 : 0.1115 / 256 * gain );
...
blargg_err_t Vgm_Emu::setup_fm()
{
	...
	if ( ym2612_rate )
	{
		...
		double factor = Dual_Resampler::setup( fm_rate / blip_buf.sample_rate(), rolloff, gain );
	...
	{
		//dprintf( "fm_rate: %f\n", fm_rate );
		fm_time_factor = (long) floor( fm_rate * (1L << fm_time_bits) / vgm_rate + 0.5 );
...
blargg_err_t Vgm_Emu::load( const header_t& h, Data_Reader& reader )
{
	...
	memset( owned_data + data_size, 0x66, padding ); // pad with end command
