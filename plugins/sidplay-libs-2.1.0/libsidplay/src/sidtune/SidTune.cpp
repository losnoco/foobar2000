...
SidTune::SidTune(const uint_least8_t* data, const uint_least32_t dataLen, const uint_least8_t* data2, const uint_least32_t data2Len)
{
    init();
    getFromBuffer(data,dataLen, data2, data2Len);
}
...
bool SidTune::read(const uint_least8_t* data, uint_least32_t dataLen, const uint_least8_t* data2, uint_least32_t data2Len)
{
    cleanup();
    init();
    getFromBuffer(data,dataLen,data2,data2Len);
...
void SidTune::getFromBuffer(const uint_least8_t* const buffer, const uint_least32_t bufferLen)
{
	...
	{
		PP20 myPP;
		if ( myPP.isCompressed(buf1.get(),buf1.len()) )
		{
			uint_least8_t* destBufRef = 0;
			uint_least32_t tempLen;
			if ( 0 == (tempLen = myPP.decompress(buf1.get(),buf1.len(),
				&destBufRef)) )
			{
				info.statusString = myPP.getStatusString();
				return;
			...
			else
			{
				info.statusString = myPP.getStatusString();
				// Replace compressed buffer with uncompressed buffer.
				buf1.assign(destBufRef,tempLen);
			}
		}
	}
...
void SidTune::getFromBuffer(const uint_least8_t* const buffer, const uint_least32_t bufferLen,
							const uint_least8_t* const buffer2, const uint_least32_t buffer2Len)
{
    // Assume a failure, so we can simply return.
    status = false;

    if (buffer==0 || bufferLen==0 || buffer2==0 || buffer2Len==0)
    {
        info.statusString = SidTune::txt_empty;
        return;
    }
    else if (bufferLen > SIDTUNE_MAX_FILELEN || buffer2Len > SIDTUNE_MAX_FILELEN)
    {
        info.statusString = SidTune::txt_fileTooLong;
        return;
    }

    uint_least8_t* tmpBuf, * tmp2Buf;
#ifdef HAVE_EXCEPTIONS
    if ( 0 == (tmpBuf = new(std::nothrow) uint_least8_t[bufferLen]) )
#else
    if ( 0 == (tmpBuf = new uint_least8_t[bufferLen]) )
...
#ifdef HAVE_EXCEPTIONS
    if ( 0 == (tmp2Buf = new(std::nothrow) uint_least8_t[buffer2Len]) )
#else
    if ( 0 == (tmp2Buf = new uint_least8_t[buffer2Len]) )
#endif
    {
		delete tmpBuf;
        info.statusString = SidTune::txt_notEnoughMemory;
    ...
    memcpy(tmp2Buf,buffer2,buffer2Len);

    Buffer_sidtt<const uint_least8_t> buf1(tmpBuf,bufferLen);
    Buffer_sidtt<const uint_least8_t> buf2(tmp2Buf,buffer2Len);

	{
		...
		if ( myPP.isCompressed(buf2.get(),buf2.len()) )
		{
			uint_least8_t* destBufRef = 0;
			uint_least32_t tempLen;
			if ( 0 == (tempLen = myPP.decompress(buf2.get(),buf2.len(),
				&destBufRef)) )
			{
				info.statusString = myPP.getStatusString();
				return;
			...
			else
			{
				info.statusString = myPP.getStatusString();
				// Replace compressed buffer with uncompressed buffer.
				buf2.assign(destBufRef,tempLen);
			}
		}
	}
